...
- 2026-09-11: A freshly-launched run_gate5_batch.py can legitimately take ~90s of real wall-clock time between the TC-TM-02 startup warning and its first real log line (yaml_format_success) under current concurrent-fleet load -- confirmed by isolating each suspect (direct lmdb.open(), direct L2PersistentTM() construction) and finding both succeed in under 20ms, so the delay is elsewhere in build_engine() (ModelLoader/VRAMEnforcer, not yet isolated further), not a hang. Killed two processes (r4, r5) unnecessarily this session chasing what looked like zero CPU/memory movement across several Get-Process checks -- the checks themselves were only seconds apart in real time despite feeling like longer. Before concluding a freshly-launched campaign is hung, wait a genuine 90-120s (a real sleep or several spaced checks with a timestamp comparison), not a gut sense of elapsed time from tool-call count.
- 2026-09-11: Three consecutive campaign launches (r1/r2/r3, same manifest content) all instantly died on verify_environment ("dirty (1 paths)" / "SHA drift"), zero real work each time -- root cause was NOT peer contention (translator repo's own commit log showed zero other-session commits in that window): it was self-inflicted. The sequence build manifest -> discard inventory diff -> launch -> [immediately commit a taskcard_status.json bookkeeping update] moves HEAD again via that LAST commit while the just-launched detached process is still early in startup (before it reaches its own verify_environment check) -- the manifest's baked-in SHA (captured at build time) then mismatches HEAD by the time the process actually checks. Fix: commit ALL pending bookkeeping (including the "active_detached_runs" entry for the run about to launch) BEFORE calling Start-Process, never after -- then don't touch git in the translator repo again until the run's summary.json/log shows real, sustained activity (at least one accepted/failed cell), confirming it safely passed its own environment check.
- 2026-09-11: A NEW variant of the "post-launch commit races verify_environment" lesson two entries above: an UNCOMMITTED dirty path fails the check exactly the same way a moved HEAD does -- `verify_environment` checks `git status` cleanliness, not just SHA match, so it doesn't matter whether the write was ever committed. Live case: launched `wave-cellsrust-quickstart-retrigger-20260911` (pid 57940) with a clean repo, then immediately edited `taskcard_status.json` in place to fill in the real `pid`/`launched_at` (deliberately NOT committing yet, to "avoid the commit race") -- the just-started process's own check ran within that same window and died instantly on `dirty (1 paths)`. Corrected rule: after `Start-Process`, do not touch ANY tracked file in the translator repo -- committed or not -- until the run shows real progress. If you need to record the real pid, either accept `pid: null` in the committed pre-launch record (cross-reference the log file instead) or write the pid update to an untracked scratch file and fold it in only after progress is confirmed.
- 2026-09-11: `git stash push <pathspecs>` returning "No local changes to save" creates NO stash entry -- a subsequent bare `git stash pop` then pops whatever ancient stash@{0} has been sitting there (here: "pre-gitlab-merge wip" from months ago, 11 conflicted files + collisions). In this multi-session repo the pathspec'd files can become clean between your status check and the stash because a PEER commits them (happened live: TC-APT-112 wiring). Never chain `stash push ... ; stash pop` blind -- verify `git stash list` grew before ever popping; recovery is `git reset --hard HEAD` (the failed pop keeps the entry).
- 2026-09-11: tests/unit/translation_engine/test_content_type_router_frontmatter_wiring.py::test_routed_description_translation_reaches_final_translations_dict FAILS pre-existing (verified via throwaway worktree at 7eca6299, b092b687, 090341d1 -- all fail identically; NOT caused by TC-APT-110/111/112). Routed frontmatter-description translation not reaching the final doc in that scenario -- needs its own root-cause pass; do not blame it on glue-gate commits.
- 2026-09-10: Closing a heal_queue.jsonl ticket by APPENDING a new row with the same `ticket_id` and a resolved `disposition` does NOT work -- `heal_queue.py`'s readers (`open_tickets_for_source_path`, `open_ticket_locales_by_file_class`, QU-01/QU-02) evaluate every JSONL line independently with zero `ticket_id` deduplication; the original OPEN row keeps counting forever. Confirmed against `test_heal_queue_quarantine.py`'s own fixtures: every "resolved" test case writes exactly ONE row per ticket_id with the resolved disposition already in it -- update-in-place (rewrite that ticket's original line) is the real, tested contract, not append. This session had appended 17 "closure" rows across several commits before discovering the bug the hard way (a follow-up campaign instantly quarantine-rejected all 25 jobs, zero real validator calls, because the 3 tickets it thought it had closed were still open) -- fixed by rewriting the 17 originals in place, plus 59 more tickets the bug had let accumulate. Do not repeat the append pattern; write a small script that finds the ORIGINAL line by ticket_id and replaces its content.
- 2026-09-11: The detached campaign process ADOPTS its own output paths in the content repo's session ledger under the AGENT_SESSION_ID it was launched with (here `hugo-translator-fc`, the launching session's name -- NOT the mission default `d18689cf...` in taskcard_status.json). A later `session_ledger.py adopt` from a session using the mission default is therefore REFUSED ("claimed by other ACTIVE session"); that is the campaign's own claim, not a peer conflict, and `--confirm-cross-session` is the audited override. But check `git log -- <page dir>` FIRST: a peer Sonnet-5 session was committing this wave's accepted locales in pairs ~5 min after each acceptance (dc6e9fd4a4 .. 371acb7b0a), so the right move was not to commit at all and to stay on trailing review. Those peer commits verify structure only ("structural parity, 0 duplicate-link findings"); a body read caught a prose defect they missed (untranslated link text, 6/16 locales -> RB-008 MECHANISM VARIANT). Division of labour when a committer peer is live: it ships, you review and ticket.
- 2026-09-11: The dirty-tree launch hazard is CROSS-SESSION, not just self-inflicted: this session held two uncommitted tracked edits (`src/workers/campaign_runner.py` + its test) for a few minutes while a PEER session ran `Start-Process` for wave-pdfgeneratedpython-20260911 -- that wave died instantly on `translator repository is dirty (2 paths)`, and the 2 paths were mine. `verify_environment` checks the whole working tree, so in a multi-session fleet ANY session's in-progress edit blocks EVERY other session's launch. Operational rule that follows: treat a dirty translator repo as a shared outage -- make edits in one batch and commit them in the same turn, never leave tracked files modified across tool calls you do not control, and when a launch dies on 'dirty (N paths)' check `git status` for a PEER's work before assuming your own bookkeeping raced it.
