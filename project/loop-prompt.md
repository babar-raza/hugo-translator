...
- 2026-09-06 **the 12-cell commit failed attempt 1 on the session-ledger hook, and the reason indicts my earlier successes.** The hook printed `Resolution: AMBIGUOUS - 4 live sessions. Operations require --session-id or AGENT_SESSION_ID`, and HEAD did not move. **`AGENT_SESSION_ID` is unset in every shell here** -- so the 1-file nl commit and the 7-file commit did not succeed because they were correct; they succeeded because **fewer sessions were live at the time and the hook auto-resolved a single candidate**. Runbook §2 step 1 already says to export it from `taskcard_status.json` for every content-repo command; I had been relying on ambient luck instead. Retried detached with `AGENT_SESSION_ID=d18689cf-...` passed explicitly in the child env, which is now the standing pattern for every content-repo write. Worth stating plainly: a step that passes for a reason you did not verify is not a step that passed.
