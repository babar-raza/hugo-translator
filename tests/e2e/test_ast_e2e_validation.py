...
class TestASTEndToEnd:
    ...
    def test_translate_links_images(self, fixture_dir: Path, test_output_dir: Path):
        ...
        for url in urls_to_check:
            assert url in translated_content, \
                f"URL '{url}' not preserved in translation"
        ...
        assert '<https://support.aspose.com>' in translated_content, \
            "Autolink not preserved"
    ...
    def test_translate_formatting(self, fixture_dir: Path, test_output_dir: Path):
        ...
        assert '***' in translated_content, \
            "Bold italic markers (***) not preserved"
    ...
    def test_multiple_fixtures_batch(self, fixture_dir: Path, test_output_dir: Path):
        ...
        for name, result in results.items():
            assert result["length"] > 100, \
                f"{name}: Output too short ({result['length']} chars)"
