...
class TestCLIBasicInvocation:
    ...
    def test_cli_accepts_minimal_valid_args(self):
        ...
        assert result.returncode == 0, \
            "CLI with --site and --help should succeed"
    ...
    def test_cli_validation_mode_flag_parsing(self):
        ...
        for mode in ['strict', 'normal', 'lenient', 'off']:
            result = subprocess.run(
                [PYTHON_EXE, '-m', 'src.cli', '--site', 'test',
                 '--validation-mode', mode, '--help'],
                capture_output=True,
                text=True,
                timeout=30
            )
            assert result.returncode == 0, \
                f"CLI should accept validation-mode={mode}"

    def test_cli_invalid_validation_mode_rejected(self):
        """Verify invalid validation-mode values are rejected."""
        result = subprocess.run(
            [PYTHON_EXE, '-m', 'src.cli', '--site', 'test',
             '--validation-mode', 'invalid-mode', '--help'],
            capture_output=True,
            text=True,
            timeout=30
        )
        ...
        assert result.returncode != 0, \
            "CLI should reject invalid validation-mode value"
    ...
    def test_cli_max_retries_accepts_integers(self):
        ...
        assert result.returncode == 0, \
            "CLI should accept integer --max-retries"

    def test_cli_max_retries_rejects_non_integers(self):
        """Verify --max-retries rejects non-integer values."""
        result = subprocess.run(
            [PYTHON_EXE, '-m', 'src.cli', '--site', 'test',
             '--max-retries', 'not-a-number', '--help'],
            capture_output=True,
            text=True,
            timeout=30
        )
        assert result.returncode != 0, \
            "CLI should reject non-integer --max-retries"
...
class TestCLIFlagCombinations:
    ...
    def test_multiple_validation_flags_combined(self):
        ...
        assert result.returncode == 0, \
            "CLI should accept multiple validation flags"
    ...
    def test_resume_flags_combined(self):
        ...
        assert result.returncode == 0, \
            "CLI should accept resume flags combination"
    ...
    def test_terminology_flags_combined(self):
        ...
        result = subprocess.run(
            [PYTHON_EXE, '-m', 'src.cli', '--site', 'test',
             ...
             '--terminology-config', '/tmp/terminology.yaml',
             '--help'],
            capture_output=True,
            text=True,
            timeout=30
        ...
        assert result.returncode == 0, \
            "CLI should accept terminology flags combination"
...
class TestCLIExecutionAbort:
    ...
    def test_cli_with_nonexistent_config_fails_gracefully(self):
        ...
        result = subprocess.run(
            [PYTHON_EXE, '-m', 'src.cli', '--site', 'nonexistent-test-site-12345',
             '--config-root', '/tmp/nonexistent-config'],
            capture_output=True,
            text=True,
            timeout=30
        )
        ...
        assert result.returncode != 0, \
            "CLI should fail with nonexistent config"

        # Should have some error output (either stdout or stderr)
        output = (result.stdout + result.stderr).lower()
        assert len(output) > 0, \
            "CLI should produce error output for missing config"
