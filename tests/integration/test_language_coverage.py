...
MULTILINGUAL_LANGUAGES = [
    lang for lang in ALL_TARGET_LANGUAGES if lang not in SPECIALIZED_LANGUAGES
]
...
class TestMultilingualFallback:
    ...
    def test_multilingual_fallback_emits_info_log(
        ...
        with caplog.at_level(logging.INFO):
            ...
            assert len(fallback_logs) > 0, f"Missing fallback INFO log for en→{target_lang}"
    ...
    def test_multilingual_models_support_all_pairs(self, production_registry):
        ...
        multilingual_models = [m for m in models if "m2m100" in m.model_id or "nllb" in m.model_id]

        assert len(multilingual_models) > 0, "No multilingual models found in production registry"
