...
from scripts.ops.rollback import (
    BackupCreator,
    GitManager,
    RollbackExecutor,
    RollbackManager,
...
@patch.object(
    GitManager, "get_diff_summary", return_value=["file1.py | 10 +++", "file2.py | 5 ---"]
)
@patch.object(GitManager, "is_clean_checkout", return_value=True)
def test_rollback_executor_plan_rollback(
    mock_clean, mock_diff, mock_resolve, mock_branch, mock_commit
):
...
def test_rollback_executor_execute_rollback(mock_backup, mock_commit, mock_checkout, mock_clean):
...
def test_rollback_manager_rollback_to_commit(mock_verify, mock_execute, mock_plan):
    ...
    # Setup mocks
    plan = RollbackPlan(
        current_commit="abc123", target_commit="def456", current_branch="main", target_ref="def456"
    )
...
def test_rollback_manager_dry_run(mock_execute, mock_plan):
    ...
    # Setup mocks
    plan = RollbackPlan(
        current_commit="abc123", target_commit="def456", current_branch="main", target_ref="HEAD~1"
    )
    mock_plan.return_value = plan
