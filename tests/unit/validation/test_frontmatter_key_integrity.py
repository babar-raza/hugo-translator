...
"""
        ...
        assert len(extra_key_issues) > 0, (
            "Expected issue for extra hallucinated keys step3/step4"
        )
...
class TestTypeDrift:
    ...
    def test_tc_fki_06_boolean_must_not_become_string(self):
        ...
        assert len(type_issues) > 0, (
            "Expected type_drift issue for boolean→string conversion"
        )
...
class TestYAMLParseFailure:
    ...
    def test_tc_fki_07_czech_rc3_collapsed_yaml(self):
        ...
        assert len(parse_issues) > 0, (
            "Expected yaml_parse_failure for collapsed YAML blob"
        )
