...
def _write_required_config(root: Path) -> None:
    paths = [
        ...
        *[Path("config/site_profiles") / f"{site}.yaml" for site, _ in FOLDER_SURFACES],
    ]
...
def test_manifest_repin_preserves_explicit_preoutput_baseline(tmp_path, monkeypatch):
    ...
    manifest = builder.build_manifest(
        ...
        accepted_output_hashes={output: builder.sha256_file(target)},
    )
...
def test_manifest_builder_rejects_dirty_translator_repo(tmp_path, monkeypatch):
    content = tmp_path / "content"
    translator = tmp_path / "translator"
    _init_git_repo(content)
    _init_git_repo(translator)
    _patch_minimal_manifest_inputs(monkeypatch, content, translator)
    (translator / "uncommitted.txt").write_text("dirty", encoding="utf-8")

    with pytest.raises(RuntimeError, match="translator_dirty=1"):
        builder.build_manifest(
            content_repo=content,
            translator_repo=translator,
            campaign_id="pilot",
...
def test_manifest_builder_rejects_dirty_content_repo_by_default(tmp_path, monkeypatch):
    content = tmp_path / "content"
    translator = tmp_path / "translator"
    _init_git_repo(content)
    _init_git_repo(translator)
    _patch_minimal_manifest_inputs(monkeypatch, content, translator)
    (content / "baseline.txt").write_text("dirty", encoding="utf-8")

    with pytest.raises(RuntimeError, match="content_dirty=1"):
        builder.build_manifest(
            content_repo=content,
            translator_repo=translator,
            campaign_id="pilot",
