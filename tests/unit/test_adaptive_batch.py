...
def test_batch_stats_tracker():
    ...
    config = {
        ...
        "language_overrides": {
            ...
            "ru": {"initial_batch_size": 8, "min_batch_size": 4, "max_batch_size": 25},
        },
    }
    ...
    print(
        f"   Bulgarian batch size after adaptation: {new_size} (should be <8 due to high fallback rate)"
    )

    print("\n5. Simulating success for German:")
    for i in range(5):
        tracker.record_batch_result(
            language="de", batch_size=20, success=True, fallback_reason=None
        )
