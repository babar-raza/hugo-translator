...
class TestBenchmarkRunner:
    ...
    def test_run_benchmark_success(self, mock_loader_class, mock_load_corpus, mock_registry):
        ...
        # Mock backend
        mock_backend = Mock()
        mock_backend.translate_with_token_counts = Mock(return_value=(["Привет"], 10, 8))
    ...
    @patch("src.benchmarking.runner.load_corpus")
    @patch("src.benchmarking.runner.ModelLoader")
    def test_run_benchmark_with_db_persistence(
        self, mock_loader_class, mock_load_corpus, mock_registry
    ):
    ...
    @patch("src.benchmarking.runner.load_corpus")
    @patch("src.benchmarking.runner.ModelLoader")
    def test_run_benchmark_with_max_samples(
        self, mock_loader_class, mock_load_corpus, mock_registry
    ):
        """Test limiting corpus with max_samples."""
        # Setup mocks with larger corpus
        mock_load_corpus.return_value = [
            {"id": f"test_{i:03d}", "text_en": f"Text {i}", "domain": "general"} for i in range(10)
        ]
        ...
        mock_backend = Mock()
        mock_backend.translate_with_token_counts = Mock(return_value=(["Текст"] * 3, 30, 24))
    ...
    @patch("src.benchmarking.runner.load_corpus")
    @patch("src.benchmarking.runner.ModelLoader")
    def test_run_benchmark_translation_error(
        self, mock_loader_class, mock_load_corpus, mock_registry
    ):
    ...
    @patch("src.benchmarking.runner.load_corpus")
    @patch("src.benchmarking.runner.ModelLoader")
    def test_run_benchmark_multiple_batch_sizes(
        self, mock_loader_class, mock_load_corpus, mock_registry
    ):
        """Test benchmark with multiple batch sizes and iterations."""
        # Setup mocks
        mock_load_corpus.return_value = [
            {"id": f"test_{i:03d}", "text_en": f"Text {i}", "domain": "general"} for i in range(4)
        ]
        ...
        expected_results = (
            (2 // 2 * 2 + 4 // 4 * 1) * 2 * 4
        )  # Simplified: (2+1)*2*4 = 24 results total
...
def test_gpu_memory_tracking_enabled_on_cuda(mock_registry, mock_backend):
    ...
    with (
        patch("src.benchmarking.runner.TORCH_AVAILABLE", True),
