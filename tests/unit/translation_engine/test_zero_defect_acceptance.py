...
def _accepted(tmp_path: Path) -> AcceptedTranslation:
    return AcceptedTranslation.from_text(
        ...
        target_lang="es",
        gate_results={
            gate_id: {"passed": True, "action": "test", "error": None} for gate_id in range(1, 45)
        },
        campaign_id="pilot",
    )
...
def test_receipt_failure_removes_output(tmp_path):
    engine = TranslationEngine.__new__(TranslationEngine)
    accepted = _accepted(tmp_path)
    engine.campaign_context = {"receipt_sink": MagicMock(side_effect=OSError("ledger full"))}
...
def test_warn_gate_is_blocking_under_zero_defect():
    ...
    gate._run_content_gates("source", "translation", "es", Path("page.md"), None, result)
...
def test_engine_builder_rejects_force_accept_under_zero_defect():
    ...
    with pytest.raises(ValueError, match="prohibits force_accept"):
        builder._init_core_state(MagicMock(), {**builder._p})
...
def _candidate_acceptance_engine(*, validation_warnings: int = 0):
    ...
    engine.decision_engine = SimpleNamespace(
        make_decision=MagicMock(return_value=SimpleNamespace(decision=ValidationDecision.ACCEPT))
    )
    ...
    engine._write_gate = SimpleNamespace(evaluate_zero_defect=MagicMock(return_value=gates))
