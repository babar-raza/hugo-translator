...
class TestSentenceOnlySafeBehavior:
    ...
    def test_plain_paragraph_extracted_as_sentence(
...
"""
        ...
        assert len(translatable_units) >= 2, \
            "Bold formatting should trigger leaf-level extraction"

    def test_paragraph_with_link_uses_leaf_extraction(
        self, parser, extractor_sentence_only
    ):
        """
...
"""
        ...
        translatable_units = [u for u in plan.units if not u.do_not_translate]
        assert len(translatable_units) >= 2, \
            "Link should trigger leaf-level extraction"

    def test_paragraph_with_code_uses_leaf_extraction(
        self, parser, extractor_sentence_only
    ):
        """
...
"""
        ...
        for unit in translatable_units:
            ...
            assert "{{<" not in unit.source_text, \
                f"Shortcode leaked: {unit.source_text}"
...
class TestAdaptiveStrategyUnchanged:
    ...
    def test_adaptive_plain_paragraph_extracted_as_sentence(
        self, parser, extractor_adaptive
    ):
    ...
    def test_adaptive_formatted_paragraph_uses_leaf(
        self, parser, extractor_adaptive
    ):
...
class TestLeafOnlyStrategyUnchanged:
    ...
    def test_leaf_only_always_extracts_leaves(
        self, parser, extractor_leaf_only
    ):
