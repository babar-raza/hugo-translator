...
def analyze_document_pair(source_path: Path, target_path: Path) -> FailureMetrics:
    ...
    try:
        ...
        # Analyze source
        (metrics.source_links, metrics.source_code_blocks, metrics.source_code_spans,
         metrics.source_bold, metrics.source_italic, metrics.source_paragraphs, _) = \
            count_markdown_elements(source_content)

        # Analyze target
        (metrics.target_links, metrics.target_code_blocks, metrics.target_code_spans,
         metrics.target_bold, metrics.target_italic, metrics.target_paragraphs,
...
def aggregate_metrics(metrics_list: List[FailureMetrics]) -> AggregateMetrics:
    ...
    for m in metrics_list:
        if not m.parsing_errors:
            ...
            if (m.target_code_blocks < m.source_code_blocks or
                m.target_code_spans < m.source_code_spans):
            ...
            if (m.target_bold < m.source_bold or
                m.target_italic < m.source_italic):
...
def format_report(agg: AggregateMetrics, sample_size: int) -> str:
    ...
    report_lines = [
        ...
        f"**Source Corpus**: `kb.aspose.net/slides/en` → `kb.aspose.net/slides/de`",
        "",
        "## Executive Summary",
        "",
    ]
    ...
    report_lines.extend([
        ...
        "",
        "**Key Findings**:",
        f"- {broken_links_pct:.0f}% of files have broken or missing links" if agg.analyzed_successfully > 0 else "",
        f"- {missing_code_pct:.0f}% of files have missing code elements" if agg.analyzed_successfully > 0 else "",
        f"- {formatting_loss_pct:.0f}% of files have lost formatting" if agg.analyzed_successfully > 0 else "",
        f"- {structure_drift_pct:.0f}% of files have significant structure drift" if agg.analyzed_successfully > 0 else "",
        "",
    ]
...
def main():
    ...
    # Paths
    source_dir = Path(r"c:\Users\prora\OneDrive\Documents\GitHub\aspose.net\content\kb.aspose.net\slides\en")
    target_dir = Path(r"c:\Users\prora\OneDrive\Documents\GitHub\aspose.net\content\kb.aspose.net\slides\de")
    ...
    # Write report
    output_path = Path(__file__).parent.parent / "tests" / "fixtures" / "hp06" / "BASELINE_FAILURES.md"
