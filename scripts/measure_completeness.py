...
def get_output_path_file_based(source_path: Path, target_lang: str, pattern: str) -> Path:
    ...
    output_filename = pattern.format(filename=stem, lang=target_lang, ext=ext, path=source_path.name)
    return source_path.parent / output_filename


def get_output_path_folder_based(source_path: Path, target_lang: str, source_lang: str) -> Path | None:
...
def print_table(rows: list[dict], show_missing_files: bool = False) -> None:
    ...
    if not rows:
        print("No results (check that ASPOSE_NET_CONTENT or ASPOSE_ORG_CONTENT is set and content repos are accessible).")
        return
    ...
    header = f"{'Site':<30} {'Lang':<6} {'Source':>8} {'Translated':>12} {'Missing':>9} {'Coverage':>9}"
    sep = "-" * len(header)
    ...
    for site_id, site_rows in sorted(by_site.items()):
        ...
        site_coverage = round(total_translated / total_possible * 100, 1) if total_possible > 0 else 0.0
        print(sep)
        print(f"{'  TOTAL':<30} {'all':<6} {total_source:>8} {total_translated:>12} {total_missing:>9} {site_coverage:>8.1f}%")
...
def print_summary(rows: list[dict]) -> None:
    ...
    overall_pct = round(total_translated / total_source_x_lang * 100, 1) if total_source_x_lang > 0 else 0.0
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Measure translation completeness across aspose.net sites")
    ...
    parser.add_argument("--json", action="store_true", help="Output raw JSON (no missing_files list)")
    parser.add_argument("--json-full", action="store_true", help="Output full JSON including missing file lists")
    parser.add_argument("--summary", action="store_true", help="Print only the overall summary totals")
    parser.add_argument("--missing", action="store_true", help="Show first 5 missing files per lang (table mode)")
    parser.add_argument(
        "--profiles-dir",
    ...
    if not profiles:
        print(f"No site profiles found in {profiles_dir}" + (f" matching '{args.site}'" if args.site else ""), file=sys.stderr)
        sys.exit(1)
