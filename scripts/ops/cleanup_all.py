...
def find_cache_directories(project_root: Path) -> list[Path]:
    ...
    # Temp directories
    temp_dirs = [project_root / "tmp", project_root / "temp", project_root / ".tmp"]
...
def confirm_cleanup(plan: dict):
    ...
    if plan["caches"]:
        print(
            f"Cache directories to remove: {len(plan['caches'])} ({plan['caches_size_mb']:.1f} MB)"
        )
...
def cleanup_caches(cache_paths: list[Path]) -> dict:
    ...
    return {"removed": removed, "failed": failed, "size_freed_mb": size_freed / (1024 * 1024)}
...
def cleanup_models(models, disk_manager, inventory_manager=None) -> dict:
    ...
    return {"removed": removed, "failed": failed, "size_freed_mb": size_freed}
...
def main():
    ...
    parser.add_argument(
        "--min-size", type=float, default=100, help="Minimum model size to remove (MB)"
    )
    ...
    if args.dry_run:
        if args.json:
            ...
            if plan["models"]:
                print(
                    f"Would remove {len(plan['models'])} model(s): {plan['models_size_gb']:.2f} GB"
                )
            if plan["caches"]:
                print(
                    f"Would remove {len(plan['caches'])} cache(s): {plan['caches_size_mb']:.1f} MB"
                )
            print()
            total_gb = plan["models_size_gb"] + (plan["caches_size_mb"] / 1024)
            print(f"Total space to be freed: {total_gb:.2f} GB")
    ...
    if cache_paths:
        ...
        print(
            f"✓ Removed {results['caches']['removed']} cache(s), "
    ...
    if models_to_remove:
        ...
        print(
            f"✓ Removed {results['models']['removed']} model(s), "
