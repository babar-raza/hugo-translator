...
def orchestrate(
    ...
    benchmark_langs: Optional[List[str]] = None,
    ...
    e2e_content_root: Optional[Path] = None,
    e2e_output_root: Optional[Path] = None,
    e2e_langs: Optional[List[str]] = None,
    ...
    if not skip_corpus:
        ...
        corpus_args = [
            ...
            '--target-samples', str(corpus_samples),
        ]
        if content_root:
    ...
    if not skip_e2e and e2e_content_root and e2e_output_root:
        ...
        e2e_args = [
            '--site', e2e_site,
            '--content-root', str(e2e_content_root),
            '--output-root', str(e2e_output_root),
            '--langs'] + e2e_langs + [
            '--model-policy', e2e_model_policy,
            '--run-dir', str(run_dir),
        ]
    ...
    logger.info(f"3. Update production config based on recommendations")
...
Examples:
  ...
  python scripts/orchestrate_benchmark_e2e.py \\
    ...
    parser.add_argument('--content-root', type=Path,
                       help='Content root for corpus creation')
    parser.add_argument('--corpus-samples', type=int, default=200,
                       help='Target samples for stratified corpus')

    # Benchmark matrix options
    parser.add_argument('--benchmark-langs', nargs='+',
                       help='Languages for benchmark (default: all)')
    parser.add_argument('--benchmark-models', nargs='+',
                       help='Models for benchmark (default: all)')
    parser.add_argument('--benchmark-devices', nargs='+', choices=['cpu', 'cuda'],
    ...
    # E2E options
    parser.add_argument('--e2e-site', default='docs.aspose.net',
                       help='Site identifier for E2E')
    parser.add_argument('--e2e-content-root', type=Path,
                       help='Content root for E2E translation')
    parser.add_argument('--e2e-output-root', type=Path,
                       help='Output root for E2E translation (git repo)')
    parser.add_argument('--e2e-langs', nargs='+',
                       help='Languages for E2E (default: es, fr)')
    parser.add_argument('--e2e-model-policy', default='fixed',
                       choices=['fixed', 'benchmark-best', 'fallback-chain'],
                       help='Model selection policy for E2E')

    # Control flags
    parser.add_argument('--skip-corpus', action='store_true',
                       help='Skip corpus creation')
    parser.add_argument('--skip-benchmark', action='store_true',
                       help='Skip benchmark matrix')
    parser.add_argument('--skip-e2e', action='store_true',
                       help='Skip E2E pipeline')
    parser.add_argument('--skip-report', action='store_true',
                       help='Skip report generation')
    parser.add_argument('--resume-benchmark', action='store_true',
                       help='Resume benchmark from checkpoint')
