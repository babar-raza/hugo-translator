    ...
    if args.json:
        ...
        if args.by_backend:
            output["usage_by_backend"] = {
                backend: round(size, 2)
                for backend, size in manager.get_usage_by_backend().items()
            }

        if args.by_model:
            output["models"] = [
                m.to_dict() for m in manager.get_models_usage()
            ]
        ...
        if args.cleanup_candidates:
            ...
            if candidates:
                ...
                print()
                print("  Run scripts/cleanup_models.py to remove unused models")
            else:
