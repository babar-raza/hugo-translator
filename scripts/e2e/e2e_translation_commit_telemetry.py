...
def select_model_by_policy(
    ...
    if policy == "fixed":
        ...
        if not benchmark_db_path or not benchmark_db_path.exists():
            logger.warning("Benchmark DB not found, falling back to fixed model")
            return fixed_model
...
def write_e2e_report(report: E2EReport, output_path: Path):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write("# E2E Translation Pipeline Report\n\n")
        ...
        f.write("## Telemetry Records\n\n")
        f.write(
            "| Language | Model | Device | Batch Size | Segments/sec | Files Modified | Commit |\n"
        )
        f.write(
            "|----------|-------|--------|------------|--------------|----------------|--------|\n"
        )
        ...
        for t in report.telemetry_records:
            ...
            f.write(
                f"| {t.language} | {t.model_id} | {t.device} | {t.batch_size} | "
...
def main():
    parser = argparse.ArgumentParser(
        description="E2E translation pipeline with git commit and telemetry"
    )
    ...
    parser.add_argument(
        "--output-root", type=Path, required=True, help="Path to output directory (git repo)"
    )
