...
def run_full_translation():
    ...
    # Source directory
    source_dir = Path(
        r"D:\onedrive\Documents\GitHub\aspose.net\content\products.aspose.net\slides\en"
    )
    ...
    for file_idx, md_file in enumerate(md_files, 1):
        ...
        try:
            ...
            if result.success:
                ...
                print(
                    f"   Segments: {result.stats.total_segments}, TM: {result.stats.tm_hits}, Model: {result.stats.translated_segments}"
                )
                print(
                    f"   Tokens: {result.stats.tokens_input} in / {result.stats.tokens_output} out"
                )
