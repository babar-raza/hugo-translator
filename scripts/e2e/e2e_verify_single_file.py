...
def check_bold_to_list_corruption(
    source_ast: list[ASTNode], target_ast: list[ASTNode]
) -> list[str]:
    """
    ...
    def is_bold_only_paragraph(node: ASTNode) -> bool:
        ...
        non_ws_children = [
            c for c in node.children if not (c.type == NodeType.TEXT and not c.raw.strip())
        ]
    ...
    for bold_text in source_bold_paras:
        if has_list_items_near_text(target_ast, bold_text):
            issues.append(
                f"Bold-only paragraph appears to be converted to list item: '{bold_text[:50]}...'"
            )

    return issues


def verify_ast_structural_fidelity(source_content: str, target_content: str) -> dict[str, any]:
    """
    ...
    try:
        ...
        for level in set(
            list(source_counts["headings"].keys()) + list(target_counts["headings"].keys())
        ):
...
def generate_untranslated_breakdown(
    source_content: str, target_content: str, lang: str, output_path: Path | None = None
) -> dict:
    """
    ...
    """
    if not HUGO_PARSER_AVAILABLE:
        return {"available": False, "note": "HugoParser not available, breakdown disabled"}
...
def check_excessive_untranslated(source: str, target: str, lang: str) -> tuple[bool, float]:
    ...
    if HUGO_PARSER_AVAILABLE:
        try:
            ...
            print("Falling back to simple word-based check", file=sys.stderr)
...
def verify_file(source_path: Path, target_path: Path, lang: str) -> dict:
    ...
    result["checks"]["code_fences"] = {
        ...
        "passed": (
            source_fences["count"] == target_fences["count"] and target_fences["is_balanced"]
        ),
    }
    ...
    lists_passed = (
        source_lists["unordered"] == target_lists["unordered"]
        and source_lists["ordered"] == target_lists["ordered"]
    )
    ...
    result["checks"]["token_leakage"] = {"leaks": leaks, "passed": len(leaks) == 0}
    ...
    result["checks"]["terminology"] = {"issues": term_issues, "passed": len(term_issues) == 0}
    ...
    if os.environ.get("VERIFIER_DEBUG_UNTRANSLATED") == "1":
        breakdown_dir = Path(
            "reports/translation_hardening/20260128-153000/phase_untranslated_ratio"
        )
        ...
        if breakdown.get("available"):
            ...
            print(
                f"[DEBUG] Untranslated chars: {breakdown['total_untranslated_chars']}",
                file=sys.stderr,
            )
            print(
                f"[DEBUG] Untranslated ratio: {breakdown['untranslated_ratio']:.1%}",
                file=sys.stderr,
            )
            print("[DEBUG] Top 5 contributors:", file=sys.stderr)
            for contrib in breakdown["contributors"][:5]:
                print(
                    f"  - {contrib['node_type']}: {contrib['source_chars']} chars "
                    f"(translatable={contrib['translatable']})",
                    file=sys.stderr,
                )
