...
def run_translation_with_telemetry():
    ...
    try:
        telemetry = configure_telemetry(agent_name="hugo-translator-slides", enabled=True)
    ...
    try:
        ...
        tm = TranslationMemory(
            l1_cache=l1_cache, l2_persistent=l2_persistent, l3_semantic=l3_semantic
        )
            ...
            for file_idx, md_file in enumerate(md_files, 1):
                ...
                try:
                    ...
                    if result.success:
                        ...
                        print(
                            f"   Segments: {result.stats.total_segments}, TM: {result.stats.tm_hits} (L1:{result.stats.l1_hits} L2:{result.stats.l2_hits} L3:{result.stats.l3_hits}), Model: {result.stats.translated_segments}"
                        )
                        print(
                            f"   Tokens: {result.stats.tokens_input} in / {result.stats.tokens_output} out / {result.stats.tokens_cached} cached"
                        )

                        # Update locale stats
                        for locale in TARGET_LOCALES:
                            results["locale_stats"][locale]["files"] += 1
                            results["locale_stats"][locale]["segments"] += (
                                result.stats.total_segments // len(TARGET_LOCALES)
                            )
                    else:
                ...
                if file_idx > 0:
                    ...
                    print(
                        f"   Progress: {file_idx}/{len(md_files)} files, Elapsed: {elapsed:.1f}s, ETA: {eta:.1f}s"
                    )
    ...
    if results["total_tokens_in"] + results["total_tokens_out"] > 0:
        cache_rate = (
            results["total_tokens_cached"]
            / (results["total_tokens_in"] + results["total_tokens_out"])
            * 100
        )
...
if __name__ == "__main__":
    ...
    if results:
        ...
        success_rate = (
            results["successful"] / results["expected_outputs"] * 100
            if results["expected_outputs"] > 0
            else 0
        )
