...
def main():
    parser = argparse.ArgumentParser(
        description="Split manifest file into deterministic batches"
    )
    parser.add_argument(
        '--input',
        type=Path,
        required=True,
        help='Input manifest file (one path per line)'
    )
    parser.add_argument(
        '--output-dir',
        type=Path,
    ...
    parser.add_argument(
        ...
        type=str,
        default='batch',
        help='Prefix for batch filenames (default: batch)'
    )
    parser.add_argument(
        '--allow-missing',
        action='store_true',
    ...
    if args.batch_size < 1:
        print(f"ERROR: Batch size must be >= 1")
        sys.exit(1)
