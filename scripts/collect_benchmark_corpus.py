...
def parse_token_range(token_range_str: str) -> Tuple[int, int]:
    ...
    if len(parts) != 2:
        raise ValueError(f"Invalid token range format: {token_range_str}. Expected format: '50-500'")
    ...
    if min_tokens < 0 or max_tokens < min_tokens:
        raise ValueError(f"Invalid token range: {token_range_str}. Min must be >= 0 and max must be >= min")
  ...
  python scripts/collect_benchmark_corpus.py \\
    ...
    try:
        ...
        for sample in metadata['samples'][:5]:
            logger.info(f"  {sample['id']}: {sample['category']} ({sample['tokens']} tokens) - {sample['path']}")
