...
class MatrixCheckpoint:
    ...
    def save(self, path: Path):
        ...
        logger.info(f"Saved checkpoint: {len(self.completed_jobs)}/{self.total_jobs} jobs completed")
...
def filter_models_by_language_pair(
    models: List[Dict],
    src_lang: str,
    tgt_lang: str
) -> List[Dict]:
...
def run_matrix_benchmark(
    ...
    logger.info(f"Matrix configuration:")
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"MATRIX BENCHMARK COMPLETE")
    logger.info(f"{'='*60}")
...
def main():
    ...
    parser.add_argument('--corpus-source', default='json', choices=['json', 'markdown'], help='Corpus source')
    parser.add_argument('--batch-sizes', nargs='+', type=int, default=[4, 8, 16], help='Batch sizes')
