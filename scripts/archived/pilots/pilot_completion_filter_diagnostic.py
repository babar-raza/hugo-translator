...
# No replacement needed — one-time diagnostic.
"""
...
# ── BEFORE state ─────────────────────────────────────────────────────────────
print(f"\n{'─' * 50}")
print("BEFORE (filter_source_files only, no completion filter):")
print(f"  Source files selected per run: {total_source_before}")
print("  With files_per_commit=20: each run processes files 1-20 (always the same)")

# ── AFTER state ──────────────────────────────────────────────────────────────
print(f"\n{'─' * 50}")
print("AFTER (with completion-aware filter, Stage 2A):")
...
# ── Alphabetical trap ────────────────────────────────────────────────────────
print(f"\n{'─' * 50}")
print("ALPHABETICAL TRAP ANALYSIS (first 20 files, sorted):")
print(f"  {first20_complete}/20 were complete")
print("  With files_per_commit=20 and NO completion filter:")
print("    → These 20 files would consume the ENTIRE max_files budget every run")
print("    → Files 21+ would never be reached until files 1-20 are all done")
print("  With completion filter:")
if first20_complete > 0:
    ...
    print("    → All first-20 are incomplete, no slots freed (all need translation)")
...
# ── Retranslate queue ────────────────────────────────────────────────────────
print(f"\n{'─' * 50}")
print("RETRANSLATE QUEUE (Stage 2C):")
...
if first20_complete > 0:
    ...
    print("    no longer consume max_files slots, later files can now be reached")
else:
    print("  Alphabetical trap: not applicable (first 20 files all incomplete)")
print()
print("  Stage 2B (L3 in-place update): verified via unit tests (37 passing)")
print(f"  Stage 2C (retranslate queue): {len(queued)} files currently queued")
print("  Stage 1 (ollama args removed): verified by static grep")
