# ARCHIVED: 2026-06-11. Sprint: class-a-fixes (commit b4fe7cf). STATUS: COMPLETE.
# No replacement needed — one-time before/after comparison proof.
"""
...
def _render_with_old_code(ast_nodes, units):
    ...
    def old_apply(self, node):
        ...
        if node.node_addr in self.applied_units:
            ...
            if has_inline:
                node.children = [
                    c for c in node.children if c.type not in (NodeType.LIST, NodeType.CODE_BLOCK)
                ]

    renderer._apply_to_node = types.MethodType(lambda self, node: old_apply(self, node), renderer)
    nodes_copy = copy.deepcopy(ast_nodes)
    renderer.apply_translations(nodes_copy, units)
    return renderer.render_to_markdown(nodes_copy), renderer
...
def pilot_ast_renderer():
    ...
    info(
        f"Nested sub-item lines (lines starting with '  -'): OLD={len(old_subs)}, NEW={len(new_subs)}"
    )
...
def pilot_schema_migration():
    ...
    try:
        ...
        ok("v9 INSERT raises OperationalError as expected:")
    ...
    try:
        ...
        conn.close()
        ok(
            f"POST-MIGRATION INSERT succeeded: run_id={row[0]}, src_lang={row[1]}, tgt_lang={row[2]}"
        )
    except Exception as e:
...
def pilot_production_log_evidence():
    ...
    info("Date:      Apr 17 2026 (BEFORE commit b4fe7cf)")
    print()
