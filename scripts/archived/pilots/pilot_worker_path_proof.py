...
# No replacement needed — one-time proof of worker path chain.
"""
...
def main() -> int:
    ...
    if len(_metrics_ctx_calls) == 0:
        ...
        for i, call_kwargs in enumerate(_metrics_ctx_calls):
            ...
            print(
                f"    profile_filename: {profile_filename!r}  (expected: {SITE_ID + '.yaml'!r}) {'OK' if pf_ok else 'WRONG'}"
            )
            print(
                f"    family_scope:     {family_scope!r}  (expected: 'multi') {'OK' if fs_ok else 'WRONG'}"
            )
            print(
                f"    display_name:     {display_name!r}  (expected: 'Product Pages') {'OK' if dn_ok else 'WRONG'}"
            )
    ...
    for s in sidecars_all:
        ...
        if posting.get("response_code") is not None:
            print(
                f"  FAIL: HTTP POST was made for {s.name} (response_code={posting.get('response_code')})"
            )
            all_pass = False
        sidecar_data.append(
            {
    ...
    report = {
        ...
        "assertions": {
            ...
            if _metrics_ctx_calls
            else False,
            "calls_forwarded_profile_filename": all(
                c.get("profile_filename") == f"{SITE_ID}.yaml" for c in _metrics_ctx_calls
            )
            if _metrics_ctx_calls
            else False,
            ...
            "no_http_post": all(s.get("response_code") is None for s in sidecar_data),
        },
