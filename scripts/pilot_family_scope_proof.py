...
def prove_scope_resolution():
    ...
    from src.observability.metrics_scope import ScopeResolver, ScopeInput, derive_content_root_id
    ...
    for label, site_id, content_root_raw, profile_filename, family_scope, \
            expect_family, expect_product, expect_confidence in CASES:
        ...
        print(f"  reporting_confidence: {scope.reporting_confidence}  (expected: {expect_confidence})")
...
def prove_metrics_payloads():
    ...
    for label, site_id, content_root_raw, profile_filename, family_scope, \
        ...
        sidecars = sorted(evidence_path.rglob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
        scope_data = {}
        if sidecars:
...
def prove_no_total_fallback():
    ...
    from src.observability.metrics_scope import ScopeResolver, ScopeInput
    ...
    resolver = ScopeResolver()
    cases = [
        ("multi-family root (family_scope: multi)", "products.aspose.org",
         str(FIXTURE_ROOT / "products.aspose.org"), "products.aspose.org.yaml", "multi"),
        ("mixed root, no family_scope", "kb.aspose.net",
         str(FIXTURE_ROOT / "kb.aspose.net"), "kb.aspose.net.yaml", None),
        ("metrics_hints total blocked", "docs.aspose.org", "${ASPOSE_ORG_CONTENT}/docs.aspose.org",
         "docs.aspose.org.yaml", None),
    ]
    all_pass = True
...
def main():
    ...
    report_path = Path(
        "reports/family-scope-challenge-20260521/pilot-aggregate-report.json"
    )
