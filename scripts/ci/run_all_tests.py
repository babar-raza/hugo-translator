...
class PytestResultParser:
    ...
    def parse_output(output: str) -> dict[str, Any]:
        ...
        """
        results = {"total": 0, "passed": 0, "failed": 0, "skipped": 0, "errors": 0, "duration": 0.0}
...
class TestRunner:
    ...
    # Suite definitions
    SUITES = {
        "all": {"description": "Run all tests", "markers": [], "paths": []},
        "critical": {
            "description": "Run only critical tests",
            "markers": ["critical"],
            "paths": [],
        },
        "fast": {"description": "Run fast tests (<1s)", "markers": ["fast"], "paths": []},
        "slow": {"description": "Run slow tests (>1s)", "markers": ["slow"], "paths": []},
        "smoke": {"description": "Run smoke tests", "markers": ["smoke"], "paths": []},
        "unit": {
            "description": "Run unit tests",
            "paths": ["tests/unit/", "tests/tm/", "tests/models/", "tests/hardware/"],
        },
        "integration": {"description": "Run integration tests", "paths": ["tests/integration/"]},
    }
...
class TestReporter:
    ...
    def print_summary(results: list[TestResult]):
        ...
        for result in results:
            status = "[PASS]" if result.success else "[FAIL]"
            print(
                f"  {status} {result.suite_name}: {result.passed}/{result.total_tests} passed ({result.duration:.2f}s)"
            )
...
Examples:
  ...
  python scripts/run_all_tests.py --suite unit --suite integration --parallel
    ...
    parser.add_argument(
        "--timeout", type=int, default=600, help="Timeout per suite in seconds (default: 600)"
    )
    parser.add_argument(
        "--parallel",
        action="store_true",
    ...
    results = runner.execute_multiple_suites(
        suite_names=suite_names, timeout=args.timeout, parallel=args.parallel, verbose=args.verbose
    )
