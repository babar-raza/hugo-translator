...
def restore_backup(
    ...
    try:
        ...
        if metadata:
            ...
            logger.info(f"  Total Size: {metadata.get('total_size_bytes', 0) / (1024*1024):.2f} MB")
            logger.info(f"  Total Files: {metadata.get('total_files', 0)}")

            # Show components
            logger.info("Components:")
            for name, info in metadata.get('components', {}).items():
                logger.info(f"    {name}: {info.get('file_count', 0)} files, "
...
Examples:
  ...
  python scripts/restore_tm.py --backup backups/tm_20250101.tar.gz --test
    ...
    parser.add_argument(
        ...
        "-t",
        type=str,
        help="Target directory (default: restore to original locations)"
    )
    ...
    parser.add_argument(
        "--test",
        action="store_true",
        help="Test if backup is corrupted (no restoration)"
    )
