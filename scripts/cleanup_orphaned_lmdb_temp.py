...
def main(argv: list[str] | None = None) -> int:
    ...
    mode.add_argument("--dry-run", action="store_true", help="Preview candidates, delete nothing (default).")
    mode.add_argument("--apply", action="store_true", help="Actually delete orphaned LMDB directories.")
    parser.add_argument("--root", default=None, help="Override temp root (default: system temp).")
    parser.add_argument("--allow-custom-root", action="store_true", help="Allow --root outside system temp.")
    parser.add_argument("--max-age-hours", type=float, default=1.0, help="Only dirs older than N hours (default: 1).")
    parser.add_argument("--only-name", default=None, help="Restrict to LMDB dirs with this basename.")
    parser.add_argument("--verbose", action="store_true", help="Print skipped candidates with reason.")
    ...
    if args.root:
        ...
        if not args.allow_custom_root:
            try:
                ...
                print(f"ERROR: --root {root} is outside system temp {system_temp}.", file=sys.stderr)
