...
ENGLISH_PRESERVED_TOKENS = {
    "Aspose", "FOSS", "SDK", "API", "PDF", "Excel", "PowerPoint", "OneNote",
    "Word", "PPTX", "OBJ", "STL", "glTF", "COLLADA", "3MF", "FBX", "AES-256",
    "MAPI", "MSG", "Python", ".NET", "Java", "C++", "Go", "PHP", "QR", "UPC",
    "GitHub", "File Format FOSS",
}
...
def validate_file(source_path: Path, target_path: Path) -> dict:
    ...
    check(
        ...
        if set(src_families) != set(tgt_families) else "",
    )
    ...
    translate_fields = [
        "title", "description",
        "header.title", "header.subtitle", "header.image.alt_text",
        "popular_features.heading", "popular_features.text",
        "products.available_title", "products.available_subtitle",
        "products.coming_soon_title", "products.coming_soon_subtitle",
        "products.platform_suffix", "products.platform_conjunction",
        "products.platform_pair",
        "why_choose.heading", "why_choose.total_link.text",
    ]
    for field in translate_fields:
        ...
        check(
            ...
            f"missing or empty" if not val else "",
        )
    ...
    for i, (si, ti) in enumerate(zip(src_items, tgt_items)):
        ...
        has_desc = bool(ta.get("description", "").strip()) or bool(ta.get("base_description", "").strip())
