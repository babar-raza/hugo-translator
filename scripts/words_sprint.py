...
def check_tm_backup_exists() -> tuple[bool, str]:
    ...
    if not tm_backups.exists():
        return False, "TM backup directory does not exist — run scripts/backup_tm.py first"
    today = datetime.now(timezone.utc).strftime("%Y%m%d")
    today_backups = list(tm_backups.glob(f"*{today}*"))
    if not today_backups:
        return False, f"No TM backup for today ({today}) — run scripts/backup_tm.py first"
...
def run_preflight_metrics_test() -> int:
    ...
    try:
        ...
        log("  To post a live test row, use: python -m src.observability.agent_metrics_integration --test-row")
        return 0
    except Exception as e:
...
def main():
    ...
    parser.add_argument("--site", help="Only sprint this site_id (default: all surfaces in priority order)")
    parser.add_argument("--dry-run", action="store_true", help="Plan batches but do not run translation worker")
    parser.add_argument("--batch-size", type=int, default=25, help="Files per batch (default: 25)")
    parser.add_argument("--max-batches", type=int, default=0, help="Stop after N batches per surface (0=unlimited)")
    parser.add_argument("--inventory-path", help="Path to words-inventory-YYYYMMDD.jsonl (auto-discover if not set)")
    parser.add_argument("--resume-from-checkpoint", help="Resume from checkpoint JSONL file")
    parser.add_argument("--preflight-metrics-test", action="store_true", help="Only run metrics preflight test and exit")
    parser.add_argument("--hugo-build-check", action="store_true", help="Run Hugo build check after each batch")
    ...
    for surface_id in surfaces_to_run:
        ...
        while batch_start < len(pending_records):
            ...
            log(f"  Batch {batch_num} result: succeeded={succeeded} failed={failed} duration={batch_duration:.1f}s")
            ...
            if not args.dry_run:
                ...
                if not gates_ok:
                    ...
                    if hard_stops:
                        ...
                        log(f"  Resume with: python scripts/words_sprint.py --site {surface_id} --resume-from-checkpoint {checkpoint_path}")
                        break
                    else:
                        log(f"  [WARN] Soft gate failures — continuing with caution")
    ...
    if stop_reason:
        ...
        print(f"  Status: EXECUTION_PARTIAL_BLOCKED")
        return 1
    else:
