...
class InventoryManager:
    ...
    def compare_inventories(self, baseline_path: str, current_path: str) -> dict:
        ...
        modified = []
        for path in common:
            if (baseline_files[path].size_bytes != current_files[path].size_bytes or
                baseline_files[path].total_lines != current_files[path].total_lines):
...
class Reporter:
    ...
    def generate_summary(inventory: Inventory) -> str:
        ...
        for category, count in sorted(inventory.stats.files_by_category.items(),
    ...
    def print_diff(diff: dict):
        ...
        print(f"  Files: {diff['baseline']['total_files']}, Lines: {diff['baseline']['total_lines']:,}")
        print(f"Current: {diff['current']['path']}")
        print(f"  Files: {diff['current']['total_files']}, Lines: {diff['current']['total_lines']:,}")
        print()
        print("Changes:")
...
Examples:
  ...
  python scripts/inventory_files.py --scan --format markdown
    ...
    parser.add_argument('--scan', action='store_true',
                       help='Scan project and create inventory')
    parser.add_argument('--verify', metavar='INVENTORY',
                       help='Verify files against inventory')
    parser.add_argument('--diff', nargs=2, metavar=('BASELINE', 'CURRENT'),
                       help='Compare two inventories')
