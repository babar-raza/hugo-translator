...
class BenchmarkResult:
    ...
    parallel_languages: Optional[int]
    load_mode: str
...
class CLIBenchmarker:
    ...
    def generate_report(self) -> str:
        ...
        sorted_results = sorted(
            self.results,
            key=lambda r: r.throughput_file_langs_per_min,
            reverse=True
        )
...
"""
        ...
        if any(r.parallel_languages and r.parallel_languages > 0 for r in self.results):
            parallel_results = [r for r in self.results if r.parallel_languages and r.parallel_languages > 0]
            sequential_results = [r for r in self.results if not r.parallel_languages or r.parallel_languages == 0]
            ...
            if parallel_results and sequential_results:
                ...
                best_sequential = max(sequential_results, key=lambda r: r.throughput_file_langs_per_min)

                speedup = best_parallel.throughput_file_langs_per_min / best_sequential.throughput_file_langs_per_min
...
def main():
    ...
    parser.add_argument("--corpus-size", type=int, default=5, help="Number of test files (default: 5)")
    parser.add_argument("--languages", nargs="+", default=["de", "fr"], help="Target languages to test")
    parser.add_argument("--quick", action="store_true", help="Quick benchmark (fewer configs)")

    args = parser.parse_args()

    benchmarker = CLIBenchmarker(
        test_corpus_size=args.corpus_size,
    ...
    print(f"Test: {args.corpus_size} files × {len(args.languages)} languages = {args.corpus_size * len(args.languages)} translations\n")
    ...
    report_path = PROJECT_ROOT / "reports" / f"benchmark_cli_settings_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
