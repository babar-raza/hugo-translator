...
class ModelMetadata:
    ...
    supported_pairs: Optional[List] = None
    notes: str = ""
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            for category in sorted(by_category.keys()):
                ...
                for model in sorted(by_category[category], key=lambda m: m.model_id):
                    ...
                    f.write(f"    backend: transformers\n")
                    f.write(f"    model_type: {model.model_type}\n")
                    f.write(f"    model_path: {model.target_path.relative_to(self.models_dir.parent)}\n")

                    if model.supported_pairs:
                        if model.supported_pairs == ["all"]:
                            f.write(f"    supported_pairs: all\n")
                        else:
                            f.write(f"    supported_pairs:\n")
