...
class L3Benchmark:
    ...
    def generate_report(self, results: dict, output_path: Path) -> None:
        ...
        lines.append("")
        lines.append(
            f"**Entries Encoded**: {results.get('gpu', results.get('cpu', {})).get('num_entries', 'N/A')}"
        )
        lines.append("")
        ...
        if "encoding_speedup" in results:
            ...
            lines.append("")
            lines.append(
                f"- **Encoding Speedup**: {results['encoding_speedup']:.2f}x faster on GPU"
            )
        ...
        if "encoding_speedup" in results:
            if results["encoding_speedup"] > 3.0:
                lines.append(
                    "- Excellent GPU acceleration for embeddings! Use GPU for TM indexing."
                )
