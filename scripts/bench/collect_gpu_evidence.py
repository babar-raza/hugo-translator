        ...
        try:
            ...
            if gpu_info.cuda_available:
                ...
                try:
                    ...
                    try:
                        gpu_info.vram_free_mb = int(torch.cuda.mem_get_info(0)[0] / (1024 * 1024))
                    except:
                        pass
...
Examples:
  ...
  python scripts/collect_gpu_evidence.py --collect --execute "python -m src.hardware.gpu_manager --detect"
    ...
    parser.add_argument("--collect", action="store_true", help="Collect GPU evidence")
    parser.add_argument(
        "--verify",
        metavar="MODULE",
        action="append",
    ...
    evidence = collector.collect_evidence(
        verify_modules=verify_modules, execute_commands=execute_commands
    )
