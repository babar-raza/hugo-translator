...
Examples:
  ...
  python scripts/load_test.py --workers 3 --duration 30 --force
    ...
    parser.add_argument(
        "--workers", type=int, default=10, help="Number of concurrent workers (default: 10)"
    )
    ...
    parser.add_argument(
        "--files", nargs="+", type=Path, help="Test files to use (default: auto-generated)"
    )

    parser.add_argument(
        "--langs", nargs="+", default=["es", "fr"], help="Target languages (default: es fr)"
    )

    parser.add_argument(
        "--ramp-up", type=int, default=0, help="Ramp-up time in seconds (default: 0)"
    )

    parser.add_argument("--force", action="store_true", help="Force retranslation (disable TM)")

    parser.add_argument("--output", type=Path, help="Output report path (.json or .md)")
