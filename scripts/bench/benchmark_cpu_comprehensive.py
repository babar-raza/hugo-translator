...
class CPUBenchmarkRunner:
    ...
    def __init__(
        ...
        self.thread_counts = thread_counts or (
            [self._detect_optimal_threads()] if device == "cpu" else [1]
        )
    ...
    def _load_corpus(self) -> list[dict[str, Any]]:
        ...
        if not self.corpus_path or not self.corpus_path.exists():
            ...
            return [{"id": f"synthetic_{i}", "text_en": f"Sample text {i}" * 10} for i in range(10)]
    ...
    def run_all_benchmarks(self) -> list[BenchmarkRun]:
        ...
        logger.info(f"\n{'=' * 70}")
        logger.info(
            f"Completed {len(all_runs)} benchmark runs across {len(self.target_languages)} languages"
        )
        logger.info(f"{'=' * 70}")
        return all_runs
        ...
        try:
            ...
            if model_info.backend == "ctranslate2":
                if not model_info.local_path:
                    logger.warning(f"CT2 model {model_id} missing local_path - run converter first")
                    return False
        ...
        if self.device.startswith("cuda"):
            try:
                ...
                if torch.cuda.is_available():
                    ...
                    gpu_memory_gb = torch.cuda.get_device_properties(device_idx).total_memory / (
                        1024**3
                    )
            except Exception as e:
...
def main() -> int:
    ...
    thread_counts = [int(t.strip()) for t in args.threads.split(",")] if args.threads else None
        ...
        if args.languages.lower() == "all":
            ...
            if config_path.exists():
                with open(config_path) as f:
                    ...
                    target_languages = [
                        lang["iso_code"] for lang in lang_config.get("languages", [])
                    ]
            ...
            logger.info(
                f"Using {len(target_languages)} target languages: {', '.join(target_languages)}"
            )
