...
class SafetyMonitor:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "avg_cpu_pct": sum(self.cpu_samples) / len(self.cpu_samples)
            if self.cpu_samples
            else None,
        }
...
class ProductionBenchmarker:
    ...
    def _collect_corpus(self) -> list[Path]:
        ...
        print(
            f"Corpus: {len(files)} files × {len(self.target_langs)} languages = {len(files) * len(self.target_langs)} translations"
        )
        return files
    ...
    def run_benchmark(self, config: BenchmarkConfig) -> BenchmarkResult:
        ...
        result = BenchmarkResult(
            config=config, files_count=len(self.corpus), languages_count=len(self.target_langs)
        )
...
"""
            ...
            if gpu_results and cpu_results:
                ...
                speedup = (
                    best_gpu.throughput_file_langs_per_min / best_cpu.throughput_file_langs_per_min
                )
...
def main():
    ...
    benchmarker = ProductionBenchmarker(corpus_size=args.corpus_size, target_langs=args.languages)
    ...
    print(
        f"Safety thresholds: VRAM {VRAM_THRESHOLD_PCT}%, RAM {RAM_THRESHOLD_PCT}%, CPU {CPU_THRESHOLD_PCT}%\n"
    )
