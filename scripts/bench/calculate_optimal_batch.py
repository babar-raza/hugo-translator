...
def calculate_optimal_batch_size(
    ...
    if available_for_batch <= 0:
        return {
            ...
            "utilization_percent": (
                (base_vram + CUDA_RESERVED_MB + (vram_per_item * min_batch)) / total_vram_mb
            )
            * 100,
...
def main():
    parser = argparse.ArgumentParser(description="Calculate optimal batch size for GPU translation")
    parser.add_argument(
        "--model",
    ...
    print(
        f"  - Batch Data:    {VRAM_PER_ITEM[model_size][args.precision] * result['batch_size']:,.0f} MB "
        f"({result['batch_size']} items × {VRAM_PER_ITEM[model_size][args.precision]} MB)"
    )
    print(
        f"  - Total:         {result['estimated_vram_mb']:,.0f} MB ({result['estimated_vram_mb'] / 1024:.1f} GB)"
    )
    ...
    print(
        '  --input "D:\\onedrive\\Documents\\GitHub\\aspose.net\\content\\kb.aspose.net\\slides\\en" \\'
    )
    print(
        "  --target-langs ar bg cs da de el es et fa fi fr he hi hr hu id it ja ko lt lv nl pl pt ro ru sk sl sr sv th tr uk vi zh \\"
    )
