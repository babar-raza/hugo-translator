...
class TranslationBenchmark:
    ...
    def benchmark_model(
        ...
        try:
            ...
            result = {
                ...
                "gpu_memory_before_mb": (gpu_memory_before.used_mb if gpu_memory_before else None),
                "gpu_memory_after_mb": (gpu_memory_after.used_mb if gpu_memory_after else None),
                "gpu_memory_peak_mb": (
    ...
    def compare_devices(
        ...
        if "gpu" in results and "error" not in results["gpu"] and "error" not in results["cpu"]:
            speedup = (
                results["cpu"]["avg_translation_time"] / results["gpu"]["avg_translation_time"]
            )
    ...
    def generate_report(self, results: dict, output_path: Path) -> None:
        ...
        lines.append("")
        lines.append(
            f"**Model**: {results.get('gpu', results.get('cpu', {})).get('model_id', 'N/A')}"
        )
        lines.append(
            f"**Backend**: {results.get('gpu', results.get('cpu', {})).get('backend', 'N/A')}"
        )
        lines.append(
            f"**Batch Size**: {results.get('gpu', results.get('cpu', {})).get('batch_size', 'N/A')}"
        )
        lines.append("")
        ...
        if "speedup" in results:
            if results["speedup"] > 3.0:
                ...
                lines.append(
                    "- Limited GPU speedup. Consider batch size optimization or CPU inference."
                )
        ...
        if "gpu" in results and results["gpu"].get("gpu_memory_peak_mb"):
            ...
            if peak_mb > 12288:  # > 12GB
                lines.append(
                    "- High memory usage. Consider reducing batch size or using quantized models."
                )
            elif peak_mb < 4096:  # < 4GB
                lines.append(
                    "- Low memory usage. Can potentially increase batch size for better throughput."
                )
