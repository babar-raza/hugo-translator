...
class SystemComparator:
    ...
    def compare_structure(self, legacy_content: str, new_content: str) -> dict[str, bool]:
    ...
    def compare_file(self, legacy_file: Path, new_file: Path, relative_path: str) -> dict:
    ...
    def compare_all(self) -> dict:
        ...
        legacy_map = {f.relative_to(self.legacy_dir): f for f in legacy_files}
        new_map = {f.relative_to(self.new_dir): f for f in new_files}
    ...
    def generate_report(self):
        ...
        with open(self.output_report, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- **High Similarity (≥{self.threshold:.0%}):** {self.stats['high_similarity']}\n"
            )
...
def main():
    ...
    parser.add_argument("--legacy", required=True, help="Path to legacy system output directory")

    parser.add_argument("--new", required=True, help="Path to new system output directory")
