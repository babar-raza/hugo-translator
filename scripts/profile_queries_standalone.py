...
def create_test_database(db_path: str, num_runs: int = 500) -> None:
    ...
    for i in range(num_runs):
        ...
        """, (
            ...
            "testing", json.dumps(["test"]), 15.0, timestamp, json.dumps({})
        ))
...
def main():
    ...
    for r in results_sorted:
        ...
        print(f"  [{status:>4}] {r['name']:<35} {r['avg_time_ms']:>7.2f}ms ({r['row_count']:>5} rows)")

    print("\nIndex Analysis:")
    cursor = conn.execute("SELECT name, sql FROM sqlite_master WHERE type='index' AND name LIKE 'idx_%'")
