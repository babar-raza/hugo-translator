...
class RepoReorganizer:
    ...
    def phase_0_preflight(self):
        ...
        action = Action(
            phase=0,
            action_type="BACKUP",
            source=f"Create branch: {backup_branch}"
        )
        ...
        try:
            ...
            action.success = False
            action.error = f"Failed to create backup branch: {e.stderr.decode() if e.stderr else str(e)}"
...
Examples:
    ...
    parser.add_argument(
        "--report-dir",
        type=str,
        help="Directory to save reports (default: reports/migration/)"
    )
