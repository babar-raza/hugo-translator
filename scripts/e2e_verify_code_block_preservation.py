...
TEST_FILES = [
    ...
    ("blog.aspose.net/barcode/rotate-barcode-images-in-csharp-with-aspose-barcode/index.md", "ru", 7),
    ("blog.aspose.net/cells/convert-excel-worksheet-to-image-in-csharp-with-aspose-cells/index.md", "pt", 7),
...
def main():
    ...
    for rel_path, target_lang, expected_src_cb in TEST_FILES:
        ...
        try:
            ...
            print(f"  FAIL: Translation timed out (300s)")
            failed += 1
            results.append({
                "file": rel_path, "lang": target_lang,
                "status": "TIMEOUT", "src_cb": src_cb,
            })
            continue
        ...
        if output_path is None:
            ...
            results.append({
                "file": rel_path, "lang": target_lang,
                "status": "NO_OUTPUT", "src_cb": src_cb,
            })
            continue
        ...
        if issues:
            ...
            print(f"  PASS: Code blocks preserved ({src_cb} -> {out_cb}), headings OK ({src_hd} -> {out_hd})")
            passed += 1
            status = "PASS"

        results.append({
            "file": rel_path, "lang": target_lang, "status": status,
            "src_cb": src_cb, "out_cb": out_cb,
            "src_hd": src_hd, "out_hd": out_hd,
