...
def smoke_test_model(model_id: str, src_lang: str, tgt_lang: str, registry) -> dict:
    ...
    if model_info.backend in ("llm", "local_llm"):
        ...
        result["error_message"] = (
            f"LLM backend ({model_info.backend}) — no local translate, skipping"
        )
        return result
    ...
    try:
        ...
        def _translate():
            return backend_obj.translate(TEST_SENTENCE, src_lang=src_lang, tgt_lang=tgt_lang)
    ...
    for plan in FAMILY_TESTS:
        ...
        for model_id in plan["candidates"]:
                    ...
                    family_result["candidates_tried"].append(
                        {
                            ...
                            "local_path": str(model_info.local_path)
            ...
            if plan.get("skip_if_backend_in") and model_info.backend in plan["skip_if_backend_in"]:
                print(
                    f"  [{model_id}] backend={model_info.backend} — skip (LLM, needs API endpoint)"
                )
                family_result["candidates_tried"].append(
                    {
                        "model_id": model_id,
