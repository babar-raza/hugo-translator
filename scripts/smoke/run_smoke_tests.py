...
class SmokeTestRunner:
    ...
    def parse_pytest_output(self, output: str) -> dict:
        ...
        """
        stats = {"passed": 0, "failed": 0, "skipped": 0, "errors": 0, "total": 0, "duration": 0.0}
    ...
    def check_test_speed(self) -> bool:
        ...
        exit_code, output, elapsed = self.run_pytest(markers="smoke", verbose=False, parallel=True)
...
Examples:
  ...
  # Check test speed
  python scripts/smoke/run_smoke_tests.py --check-speed

  # Run with custom timeout
  python scripts/smoke/run_smoke_tests.py --full --timeout 120
""",
    )
    ...
    parser.add_argument(
        "--full", action="store_true", help="Run full smoke tests (all tests, ~50 tests, <60s)"
    )
    parser.add_argument(
        "--check-speed", action="store_true", help="Verify smoke tests meet speed requirements"
    )
    parser.add_argument(
    ...
    # Create runner
    runner = SmokeTestRunner(project_root=args.project_root, timeout=args.timeout)
