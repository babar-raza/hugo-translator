...
class ChecklistLoader:
    ...
    def _define_checklist(self) -> list[CheckItem]:
        ...
        items.extend(
            [
                CheckItem(
                    "Security", "Dependency vulnerability scan", automated=True, required=True
                ),
                CheckItem("Security", "No exposed secrets", automated=True, required=True),
                CheckItem(
                    "Security", "Dependencies at secure versions", automated=True, required=True
                ),
        ...
        items.extend(
            [
                CheckItem(
                    "Performance", "Performance baseline established", automated=True, required=True
                ),
                CheckItem(
                    "Performance", "No performance regressions", automated=True, required=True
                ),
                CheckItem(
                    "Performance", "Resource requirements met", automated=True, required=True
                ),
        ...
        items.extend(
            [
                ...
                CheckItem(
                    "Documentation", "Deployment guide current", automated=False, required=True
                ),
            ]
        )
        ...
        items.extend(
            [
                CheckItem(
                    "Infrastructure", "Production readiness check", automated=True, required=True
                ),
                CheckItem(
                    "Infrastructure", "Monitoring configured", automated=False, required=True
                ),
...
class AutomatedChecker:
    ...
    def check_smoke_tests(self) -> tuple[bool, str, str]:
        ...
        smoke_script = self.project_root / "scripts" / "smoke" / "run_smoke_tests.py"
...
class ApprovalReporter:
    ...
    def generate_report(
        self, checklist_report: ChecklistReport, output_path: Path | None = None
    ) -> str:
...
""",
    ...
    parser.add_argument(
        "--checklist", action="store_true", help="Display deployment safety checklist"
    )
    parser.add_argument("--run-checks", action="store_true", help="Run automated safety checks")
    parser.add_argument(
        "--enforce", action="store_true", help="Enforce checklist completion (exit 1 if not ready)"
    )
    parser.add_argument(
        "--approve", action="store_true", help="Generate deployment approval report"
    )
    parser.add_argument("--report", type=Path, help="Save approval report to JSON file")
    parser.add_argument("--markdown", type=Path, help="Save approval report to Markdown file")
    parser.add_argument(
        "--project-root",
        type=Path,
