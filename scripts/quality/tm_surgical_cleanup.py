...
def is_corrupt_entry(entry: TranslationEntry) -> tuple[bool, str, str]:
    """
    Determine if a TM entry is corrupt.

    Returns:
        (is_corrupt, reason, action)
        action is 'overwrite' (identity) or 'delete'
    """
...
def run(
    ...
    for entry in tm.export_iter(site_id=site):
        ...
        if len(to_overwrite) + len(to_delete) >= max_changes:
    ...
    if apply:
        print(f"\nApplying {len(to_overwrite)} overwrites and {len(to_delete)} deletes...")
...
def main() -> None:
    ...
    parser.add_argument(
        "--site", type=str, default=DEFAULT_SITE,
    ...
    parser.add_argument(
        "--max-changes", type=int, default=25000,
        help="Maximum entries to modify (safety cap, default: 25000)",
    )
    parser.add_argument(
        "--tm-path", type=Path, default=DEFAULT_TM_PATH,
    ...
    only_locales = (
        [l.strip() for l in args.locales.split(",")] if args.locales else None
    )
    ...
    if not apply:
        ...
        print("Rollback: python scripts/tm/restore_tm.py --input data/tm/backups/backup_pre_cleanup.tar.gz")
