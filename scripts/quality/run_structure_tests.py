...
def run_tests():
    ...
    try:
        ...
        if isinstance(doc.frontmatter, CommentedMap):
            ...
            results.append(
                f"✗ FAIL: Parser returns {type(doc.frontmatter).__name__}, expected CommentedMap"
            )
            failed += 1
    ...
    try:
        ...
        if isinstance(copied, CommentedMap):
            ...
            results.append(
                f"✗ FAIL: _copy_commented_map returns {type(copied).__name__}, expected CommentedMap"
            )
            failed += 1
    except Exception as e:
    ...
    try:
        ...
        if len(comments_found) == 4:
            ...
            results.append(
                f"✗ FAIL: Only {len(comments_found)}/4 comments preserved: {comments_found}"
            )
            failed += 1
    except Exception as e:
    ...
    try:
        ...
        results.append(
            f"  Comments: {result_comments}/{original_comments} ({comment_retention:.1f}% retention)"
        )
        results.append(
            f"  Literal blocks: {result_literal}/{original_literal} ({literal_retention:.1f}% retention)"
        )
...
def compare_real_files():
    ...
    try:
        ...
        en_comments = len(
            [l for l in en_lines if l.strip().startswith("#") and not l.strip().startswith("#-")]
        )
        ...
        bg_comments = len(
            [l for l in bg_lines if l.strip().startswith("#") and not l.strip().startswith("#-")]
        )
        bg_literal = bg_content.count(": |") + bg_content.count(": |-")

        results.append("\n=== REAL FILE COMPARISON ===")
        results.append(
            f"EN file: {len(en_lines)} lines, {en_comments} comments, {en_literal} literal blocks"
        )
        results.append(
            f"BG file: {len(bg_lines)} lines, {bg_comments} comments, {bg_literal} literal blocks"
        )
