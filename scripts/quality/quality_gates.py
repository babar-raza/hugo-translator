...
class DirectoryStructureGate(QualityGate):
    ...
    def _check(self) -> GateResult:
        ...
        return GateResult(
            ...
            message=f"Directory structure check {'passed' if passed else 'failed'}. Missing: {missing_dirs}"
            if missing_dirs
            else "All required directories exist",
        )
...
class SyntaxValidGate(QualityGate):
    ...
    def _check(self) -> GateResult:
        ...
        return GateResult(
            ...
            message=f"Syntax validation {'passed' if passed else 'failed'}. Invalid files: {invalid_files}"
            if invalid_files
            else "All files have valid syntax",
        )
...
class ImportHealthGate(QualityGate):
    ...
    def _check(self) -> GateResult:
        ...
        return GateResult(
            ...
            message=f"Import health {'passed' if passed else 'failed'}. Failed imports: {failed_imports}"
            if failed_imports
            else "All modules importable",
        )
...
class DocumentationCompleteGate(QualityGate):
    ...
    def _check(self) -> GateResult:
        ...
        return GateResult(
            ...
            message=f"Documentation check {'passed' if passed else 'failed'}. Missing: {missing_docs}"
            if missing_docs
            else "All required documentation exists",
        )
...
class GateRunner:
    ...
    def execute_gates(
        self, gate_filter: str | None = None, fail_fast: bool = False
    ) -> list[GateResult]:
        """
    ...
    def generate_report(
        self, results: list[GateResult], summary: dict[str, Any], output_file: Path
    ):
        """Generate JSON report of gate results."""
        report = {"summary": summary, "gates": [r.to_dict() for r in results]}
...
Examples:
  ...
  python scripts/quality_gates.py --gate test_pass_rate
    ...
    parser.add_argument(
        "--fail-fast", action="store_true", help="Stop at first critical gate failure"
    )
    parser.add_argument("--report", type=Path, help="Output JSON report to file")
    parser.add_argument("--config", type=Path, help="Path to quality gates config file")
    ...
    try:
        ...
        # Execute gates
        results = runner.execute_gates(gate_filter=args.gate, fail_fast=args.fail_fast)
