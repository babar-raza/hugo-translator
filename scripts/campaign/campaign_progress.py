...
def main() -> int:
    ...
    for batch in rows(root / "commit_batches.jsonl"):
        if batch.get("status") == "COMMITTED":
            committed.update(str(path) for path in batch.get("outputs", []))
    ...
    payload = {"campaign": manifest.campaign_id, "accepted": accepted, "failures": len(failures), "rate_per_minute": round(rate, 3), "remaining": remaining, "eta_minutes": round(remaining / rate, 1) if rate else None, "committed": len(committed), "pending_commit": max(0, accepted-len(committed)), "spool": spool, "metrics": metrics}
