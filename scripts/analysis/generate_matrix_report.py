...
def generate_report(
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write("# Final Benchmark Matrix Report\n\n")
        ...
        f.write("| Language | Top Model | Avg Throughput (tokens/sec) | Samples |\n")
        f.write("|----------|-----------|------------------------------|----------|\n")
        ...
        if recommendation["recommended_policy"] == "fallback-chain":
            f.write(
                "1. **Primary:** Use specialized models for languages with dedicated support:\n"
            )
            ...
            f.write("1. **Use single model for all languages:** `m2m100_418m_ct2`\n")
            f.write("2. **Rationale:** Consistent performance, simpler deployment\n")

        f.write("\n### Deployment Configuration\n\n")
        f.write("```yaml\n")
        f.write("model_defaults:\n")
        f.write('  fallback_model: "m2m100_418m_ct2"\n')
        f.write(f'  device: "{device}"\n')
        ...
        # Appendix
        f.write("## Appendix: Benchmark Methodology\n\n")
        f.write("- **Corpus:** Stratified sample across aspose.net subdomains and families\n")
        f.write("- **Metrics:** Average throughput (tokens/second)\n")
        f.write(f"- **Device:** {device}\n")
        f.write("- **Iterations:** 3 per benchmark\n")
...
def main():
    ...
    parser.add_argument(
        "--device", default="cuda", choices=["cpu", "cuda"], help="Device to analyze"
    )
