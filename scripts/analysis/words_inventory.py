...
def build_record(
    ...
    return {
        ...
        "recommended_worker_queue": "content_translation"
        if translation_required
        else "quality_review",
        "recommended_translation_strategy": "tm_l2_then_model"
        if translation_required
        else "quality_only",
        "validation_commands": [
        ...
        "risk_level": "low"
        if surface in ("docs", "kb")
        else "medium"
        if surface in ("blog", "products")
        else "high",
        "notes": "",
    }
...
def main():
    ...
    parser.add_argument(
        "--output", help="Output JSONL path (default: data/sprints/words-inventory-YYYYMMDD.jsonl)"
    )
    ...
    if not content_base:
        ...
        print(
            "  Example: export ASPOSE_NET_CONTENT=/d/onedrive/Documents/GitHub/aspose.net/content",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(f"\n{'=' * 70}")
    print("  ASPOSE.WORDS TRANSLATION SPRINT — INVENTORY SCAN")
    print(f"  Date: {today}")
    ...
    for profile_name, words_subdir_override in WORDS_SURFACE_PROFILES:
        ...
        raw_roots = expand_content_roots(profile.get("content_roots", []), content_base)
        if not raw_roots:
        ...
        print(
            f"  [{site_id}] surface={surface} convention={convention} locales={len(target_langs)}"
        )
