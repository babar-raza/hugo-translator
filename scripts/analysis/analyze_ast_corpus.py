...
def collect_english_files(corpus_path: Path, sites: list = None) -> list:
    ...
    for site_dir in corpus_path.iterdir():
        ...
        if site_name in DIRECTORY_BASED_SITES:
            ...
            logger.debug(
                f"Found {len([f for f in english_files if site_name in str(f)])} file-based files from {site_name}"
            )
...
class DocumentAnalyzer:
    ...
    def __init__(self):
        """Initialize analyzer."""
        self.product_name_pattern = re.compile(r"Aspose\.\w+", re.IGNORECASE)
    ...
    def analyze_document(self, file_path: Path) -> DocumentAnalysis:
        ...
        try:
            ...
            return DocumentAnalysis(file_path=str(file_path), file_size=0, line_count=0)

        lines = content.split("\n")
        analysis = DocumentAnalysis(
            file_path=str(file_path), file_size=len(content), line_count=len(lines)
        )
        ...
        analysis.has_tables_with_code = bool(
            analysis.tables_count > 0 and re.search(r"\|.*`.*`.*\|", body)
        )

        analysis.has_mixed_content = (
            analysis.code_blocks_count > 0 and analysis.lists_count > 0 and analysis.links_count > 0
        )
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Analyze AST translation validation corpus")
    parser.add_argument(
        "--corpus-size", type=int, default=100, help="Number of documents to analyze (default: 100)"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Quick analysis (10 documents, for testing)"
    )
