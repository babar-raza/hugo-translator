        ...
        with open(IMPORT_CSV, encoding="utf-8") as f:
            ...
            for row in reader:
                try:
                    ...
                    import_data[module_path] = {"count": import_count, "status": status}
...
def inventory_repository():
    ...
    for root, dirs, files in os.walk(REPO_ROOT):
        ...
        for file in files:
            ...
            elif import_status == "orphaned" and classification["module_owner"] not in [
                "tests",
                "scripts",
            ]:
...
def generate_evidence_log(inventory):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        src_files = [
            f for f in inventory if f["type"] == "source_code" and f["language"] == "python"
        ]
        ...
        f.write(
            f"   - Deprecated modules: {len([f for f in src_files if f['status'] == 'deprecated'])}\n"
        )
        f.write(
            f"   - Orphaned modules: {len([f for f in src_files if f['action'] == 'delete_candidate'])}\n\n"
        )

        f.write("2. Test Coverage:\n")
        f.write(f"   - Total test files: {len(test_files)}\n")
        f.write(
            f"   - Test fixtures: {len([f for f in inventory if f['type'] == 'data' and f['status'] == 'test_only'])}\n\n"
        )
        ...
        f.write(
            f"   - Markdown docs: {len([f for f in doc_files if f['language'] == 'markdown'])}\n"
        )
        f.write(
            f"   - Archived docs: {len([f for f in doc_files if f['status'] == 'deprecated'])}\n\n"
        )
