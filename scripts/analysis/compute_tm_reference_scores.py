...
def score_file(
    ...
    if result.tm_hit_rate < min_tm_rate:
        result.note = (
            f"TM hit rate {result.tm_hit_rate:.1%} < {min_tm_rate:.1%} threshold — skipped"
        )
        return result
...
def main() -> int:
    ...
    p.add_argument("--langs", nargs="+", default=["fr", "de"], help="Target languages to check")
    p.add_argument(
        "--n-sample", type=int, default=10, help="Files to sample per language (default: 10)"
    )
    ...
    print(f"\n{'=' * 60}")
    print("TM REFERENCE SCORE REPORT (chrF)")
    ...
    for lang in args.langs:
        ...
        if scored:
            ...
            print(
                f"  {lang:<6}  avg chrF={avg_chrf:.1f}  avg TM rate={avg_tm_rate:.1%}  ({len(scored)} files scored, {skipped} skipped)"
            )
        else:
            print(
                f"  {lang:<6}  no files met the {args.min_tm_rate:.0%} TM hit rate threshold ({skipped} skipped)"
            )

    print("\nNOTE: chrF scores are vs TM pseudo-references, not human translations.")
    print("      Lower scores may indicate model drift from established translations.")
    print(f"{'=' * 60}\n")
    return 0
