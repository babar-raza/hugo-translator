...
class TMDocReviewer:
    ...
    def find_tm_docs(self) -> list[Path]:
        ...
        tm_docs.extend((self.repo_root / "docs" / "architecture").glob("translation-memory.md"))
    ...
    def check_completeness(self, docs: list[Path]) -> list[str]:
        ...
        if arch_doc:
            ...
            if "Backup" not in content or "Recovery" not in content:
                issues.append(f"Missing required section in {arch_doc.name}: Backup & Recovery")

            for section in required_sections:
                if section not in content:
                    issues.append(f"Missing required section in {arch_doc.name}: {section}")
        ...
        for ops_doc in ops_docs:
            ...
            if "Windows" not in content or "Linux" not in content:
                issues.append(f"Missing Windows/Linux examples in {ops_doc.name}")
