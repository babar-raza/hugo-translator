...
def main():
    ...
    passing = (
        overall >= 4.0
        and all(r >= 3.0 for r in ratings.values())
        and ratings.get("correctness", 0) >= 4.0
        and ratings.get("completeness", 0) >= 4.0
    )
    ...
    if passing:
        ...
        print(
            f"   Critical dimensions ≥4.0: {'✓' if ratings.get('correctness', 0) >= 4.0 and ratings.get('completeness', 0) >= 4.0 else '✗'}"
        )
        print()
