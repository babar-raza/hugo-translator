...
def parse_time_duration(duration_str: str) -> int:
    ...
    if duration_str.endswith("h"):
        ...
        raise ValueError(
            f"Invalid duration format: {duration_str}. Use format like '1h', '30m', '24h', '7d'"
        )
...
def generate_text_report(stats: dict[str, Any], metrics: MetricsCollector) -> str:
    ...
    if trans["success_rate"] < 0.9 and trans["total"] > 10:
        recommendations.append(
            f"  - Success rate ({trans['success_rate']:.1%}) is below 90%. Check error logs."
        )
    ...
    if queue["depth"] > 1000:
        recommendations.append(
            f"  - Queue depth ({queue['depth']}) is high. Consider adding more workers."
        )
...
Examples:
  ...
  python scripts/generate_metrics_report.py --since 24h --output report.txt
    ...
    parser.add_argument(
        "--output", "-o", type=str, help="Output file path. If not specified, prints to stdout"
    )
