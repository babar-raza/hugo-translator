...
def materialize_tree(
    ...
    for line_num, line in enumerate(lines, start=1):
        try:
            ...
            try:
                ...
                print(
                    f"⚠ WARNING: File not under corpus root (line {line_num}): {source_path}",
                    file=sys.stderr,
                )
    ...
    for idx, source_path in enumerate(normalized_paths, start=1):
        ...
        try:
            ...
            print(
                f"  ⚠ Using filename only for path outside corpus root: {source_path}",
                file=sys.stderr,
            )
    ...
    if failed_files:
        ...
        print(
            f"  Continuing with {files_copied}/{files_requested} successfully copied files",
            file=sys.stderr,
        )
...
Hardening Features (2026-02-02):
  ...
  - Path normalization (absolute paths, security checks)
    ...
    parser.add_argument("--receipt", type=Path, required=True, help="Path to write JSON receipt")
    ...
    try:
        ...
        if result["failed_files"]:
            print(
                f"\n⚠ WARNING: {len(result['failed_files'])} files failed (see receipt for details)",
                file=sys.stderr,
            )
