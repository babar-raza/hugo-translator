...
def generate_markdown_report(data: dict) -> str:
    ...
    lines.append(
        f"| Extract | {seg['extract']['mean_ms']:.3f}ms | {seg['extract']['p95_ms']:.3f}ms |"
    )
    lines.append(
        f"| Reconstruct | {seg['reconstruct']['mean_ms']:.3f}ms | {seg['reconstruct']['p95_ms']:.3f}ms |"
    )
    ...
    for size in ["small", "medium", "large"]:
        s = sizes[size]
        lines.append(
            f"| {size.capitalize()} | {s['segment_count']} | {s['mean_ms']:.2f}ms | {s['ms_per_segment']:.2f}ms |"
        )
    lines.append("")
    ...
    lines.append(
        f"- **With 10k TM entries:** {mem['with_tm_mb']:.1f}MB (+{mem['tm_overhead_mb']:.1f}MB)"
    )
    ...
    lines.append(
        f"| L1 Lookups/sec | >10,000 | {l1_throughput:.0f} | {'✓' if l1_throughput > 10000 else '✗'} |"
    )

    l2_throughput = tm["layers"]["L2"]["lookups_per_sec"]
    lines.append(
        f"| L2 Lookups/sec | >1,000 | {l2_throughput:.0f} | {'✓' if l2_throughput > 1000 else '✗'} |"
    )

    pipeline_time = seg["total_pipeline"]["mean_ms"]
    lines.append(
        f"| Pipeline time | <10ms | {pipeline_time:.2f}ms | {'✓' if pipeline_time < 10 else '✗'} |"
    )
