...
class ReferenceCorpusCreator:
    ...
    def download_wmt_testset(
        self, testset: str, lang_pair: str, max_samples: int = None
    ) -> list[dict]:
        """
    ...
    def create_corpus(
        self, testsets: list[str], lang_pairs: list[str], max_samples_per_pair: int = 100
    ) -> dict[str, list[dict]]:
        """
        ...
        for lang_pair in lang_pairs:
            ...
            for testset in testsets:
                samples = self.download_wmt_testset(
                    testset, lang_pair, max_samples=max_samples_per_pair
                )
                corpus_by_lang[lang_pair].extend(samples)

        return corpus_by_lang

    def save_corpus(self, corpus_by_lang: dict[str, list[dict]], metadata: dict = None):
        """
    ...
    def create_metadata(self, testsets: list[str], lang_pairs: list[str], max_samples: int) -> dict:
        """
...
def main() -> int:
    ...
    try:
        ...
        corpus_by_lang = creator.create_corpus(
            testsets=testsets, lang_pairs=lang_pairs, max_samples_per_pair=args.samples
        )

        # Create metadata
        metadata = creator.create_metadata(
            testsets=testsets, lang_pairs=lang_pairs, max_samples=args.samples
        )
