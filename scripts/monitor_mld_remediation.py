...
def _scan_recent_logs(hours: float = 6) -> dict:
    ...
    for log_file in log_files[:10]:  # check up to 10 most recent log files
        ...
        for line in text.splitlines():
            ...
            if "SemanticSimilarity" in line and ("ERROR" in line or "BLOCK" in line or "REJECT" in line):
            ...
            if "ERROR" in line and ("Traceback" in line or "Exception" in line or "crash" in line.lower()):
...
def run_check():
    ...
    # Fix verification
    print("  RC-1 (accept_after_max_retries references):", logs["accept_after_max_retries"],
          "[OK] clean" if logs["accept_after_max_retries"] == 0 else "!! REGRESSION -- should be 0!")
    print("  RC-2 (--fast in scan):",
          "!! REGRESSION -- --fast still used!" if logs["contamination_scan_fast"] else "[OK] clean")
    print("  RC-3 (LLM escalation fired):", logs["llm_escalation_fired"],
          "-- expected >0 once queue has retry>=2 files" if logs["llm_escalation_fired"] == 0 else "[OK] firing")
    print("  SemanticSimilarity warns:", logs["semantic_similarity_warn"],
          "blocks:", logs["semantic_similarity_block"])
