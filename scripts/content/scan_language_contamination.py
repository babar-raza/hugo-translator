...
class LanguageContaminationScanner:
    ...
    def scan_repository(
        ...
        if since_commit:
            ...
            file_lang_pairs = [(fp, lang) for fp, lang in file_lang_pairs if str(fp) in changed]
        ...
        contaminated = [
            a for a in successful if a.purity_percentage < self.min_purity and a.total_sentences > 0
        ]
            ...
            try:
                ...
                if block_level and not analysis.error and not fast_mode:
                    ...
                    if analysis.contaminated_blocks:
                        ...
                        lang_counts = Counter(
                            b["detected_lang"] for b in analysis.contaminated_blocks
                        )
                        if lang_counts:
        ...
        contaminated_files = [
            {
                ...
                "dominant_lang": a.dominant_lang
                or (
                    a.wrong_lang_samples[0].get(
                        "detected", a.wrong_lang_samples[0].get("detected_lang", "")
                    )
                    if a.wrong_lang_samples
                    else ""
                ),
    ...
    def _find_translated_files_for_lang(self, repo_path: Path, lang: str) -> list[tuple[Path, str]]:
    ...
    def _check_script_mixing_unicode(self, content: str, target_lang: str) -> list[str]:
        ...
        for line in clean.splitlines():
            ...
            for word, count in counts.items():
                ...
                if count > heading_threshold:
                    issues.append(
                        f"Heading repetition: '{word}' repeated {count}× in: '{heading_text[:80]}'"
                    )
...
class ReportGenerator:
    ...
    def generate_report(self, result: ScanResult, output_path: Path) -> None:
        ...
        repetition_only = [
            ...
            for a in result.analyses
            if not a.error
            and a.repetition_issues
            and not a.script_mixing_issues
        ...
        lines.append(
            f"**Target Language:** {result.target_lang if result.target_lang != 'all' else 'all languages'}"
        )
        ...
        if result.scanned_files > 0:
            bad_total = len(
                {a.file_path for a in result.analyses if not a.error and a.has_quality_issues}
            )
        ...
        if contaminated_sorted:
            ...
            for i, analysis in enumerate(contaminated_sorted, 1):
                ...
                lines.append(
                    f"**Sentences:** {analysis.total_sentences} total, {analysis.correct_lang_count} correct"
                )
                lines.append("")
                ...
                if analysis.wrong_lang_samples:
                    ...
                    for sample in analysis.wrong_lang_samples[:3]:  # Show first 3
                        ...
                        lines.append(
                            f"- Detected: `{sample['detected']}` (confidence: {sample['confidence']:.2f})"
                        )
                        lines.append("  ```")
            ...
            lines.append("")
            lines.append(
                f"No contaminated files found. All scanned files meet the {95.0}% purity threshold."
            )
            lines.append("")
        ...
        if script_only:
            ...
            lines.append("")
            lines.append(
                "These files contain characters from an incompatible Unicode script "
        ...
        if repetition_only:
            ...
            lines.append("")
            lines.append(
                "These files contain excessive word repetition (likely model hallucination)."
            )
            lines.append("")
        ...
        if len(critical) > 0:
            ...
            lines.append("")
            lines.append(
                f"{len(critical)} file(s) have severe contamination (<50% purity). These files are mostly in the wrong language and require immediate re-translation."
            )
            lines.append("")
        ...
        if len(high) > 0:
            ...
            lines.append("")
            lines.append(
                f"{len(high)} file(s) have significant contamination (50-75% purity). Review and re-translate affected sections."
            )
            lines.append("")
        ...
        if len(medium) > 0:
            ...
            lines.append("")
            lines.append(
                f"{len(medium)} file(s) have moderate contamination (75-90% purity). Review contaminated sentences and correct as needed."
            )
            lines.append("")
        ...
        if len(low) > 0:
            ...
            lines.append("")
            lines.append(
                f"{len(low)} file(s) have minor contamination (90-95% purity). These are close to the threshold and may contain technical terms or proper nouns detected as other languages."
            )
            lines.append("")
        ...
        lines.append("")
        lines.append(
            "**Report generated by:** Language Contamination Scanner (Agent-ML-D, Task ML-004)"
        )
...
Examples:
  ...
  python scripts/scan_language_contamination.py --repo "D:\\path\\to\\content" --all-languages --since-commit abc1234
    ...
    # Print summary
    bad_total = len({a.file_path for a in result.analyses if not a.error and a.has_quality_issues})
    print("\n" + "=" * 60)
    print("SCAN COMPLETE")
    print("=" * 60)
    ...
    if bad_total > 0 and args.json_output:
        ...
        print(
            f"  python scripts/force_retranslate_contaminated.py --inventory {args.json_output} --delete-outputs"
        )
