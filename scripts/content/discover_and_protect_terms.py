    ...
    for root in content_roots:
        ...
        for md in sorted(root.rglob("*.md")):
            ...
            in_non_english_folder = any(
                lang_code_re.match(part) and part != "en" for part in md.parts
            )
...
def apply_to_terminology_yaml(
    ...
    existing_in_file: set[str] = {
        r["term"].lower() for r in data.get("global", {}).get("exact_matches", []) if "term" in r
    }
        ...
        exact_matches.append(
            {
                ...
                "severity": "info",
                "description": (
                    f"auto-discovered: freq={term.frequency}, conf={term.confidence:.2f}"
                ),
            }
        )
...
def save_pending_review(
    ...
    already_pending: set[str] = {
        entry["term"].lower() for entry in existing_data.get("pending_terms", []) if "term" in entry
    }
    ...
    if new_entries and not dry_run:
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            yaml.dump(
                existing_data, f, default_flow_style=False, allow_unicode=True, sort_keys=False
            )
...
def main() -> None:
    ...
    # ── 2. Collect corpus ────────────────────────────────────────────────────
    print(
        f"Scanning English source files across {len(content_roots)} site(s) "
    ...
    mid_conf = [t for t in fresh if args.mid_confidence <= t.confidence < args.high_confidence]
    ...
    if high_conf:
        ...
        print(
            f"\n{prefix}Auto-applied {len(added)} high-confidence terms → config/terminology.yaml:"
        )
        for t in sorted(added, key=lambda x: (-x.confidence, x.term_text)):
            print(
                f"  + {t.term_text:<30}  [{t.category:<18}]  "
    ...
    if mid_conf:
        ...
        print(f"\n{prefix}Saved {n_new} medium-confidence terms → {pending_path}:")
    ...
    if args.dry_run:
        ...
        if high_conf:
            ...
            print(
                "Restart workers (or they will pick it up on next run) for changes to take effect."
            )
        if mid_conf:
