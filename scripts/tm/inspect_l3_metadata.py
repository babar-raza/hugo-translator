...
def inspect_metadata(index_path: str = "./data/tm/l3_faiss"):
    ...
    print(
        f"✓ With valid entry_id: {entries_with_id:,} ({entries_with_id / total_entries * 100:.1f}%)"
    )

    if entries_without_id > 0:
        print(
            f"⚠️ Missing entry_id: {entries_without_id:,} ({entries_without_id / total_entries * 100:.1f}%)"
        )

    if entries_malformed_id > 0:
        print(
            f"⚠️ Malformed entry_id: {entries_malformed_id:,} ({entries_malformed_id / total_entries * 100:.1f}%)"
        )

    if entries_wrong_type > 0:
        print(
            f"⚠️ Wrong type (not dict): {entries_wrong_type:,} ({entries_wrong_type / total_entries * 100:.1f}%)"
        )
