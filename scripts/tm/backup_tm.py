...
def create_backup_metadata(tm_data_dir: Path, config_dir: Path, version: str = "1.0.0") -> dict:
    """
    ...
    metadata["total_size_bytes"] = sum(
        comp.get("size_bytes", 0) for comp in metadata["components"].values()
    )
    metadata["total_files"] = sum(
        comp.get("file_count", 0) for comp in metadata["components"].values()
    )
...
def create_backup(
    ...
    if engines:
        try:
            engines.telemetry.track_event(
                event_type="backup_started", output_path=str(output_path), compression=compression
            )
    ...
    try:
        ...
        if not tm_data_dir.exists():
            ...
            if engines:
                try:
                    engines.telemetry.track_event(event_type="backup_failed", error=error_msg)
                except Exception as e:
                    logger.debug(f"Telemetry event failed (non-fatal): {e}")
...
def rotate_backups(backup_dir: Path, keep_count: int) -> None:
    ...
    try:
        ...
        backups = sorted(
            backup_dir.glob("tm_*.tar.*"), key=lambda p: p.stat().st_mtime, reverse=True
        )
        ...
        if len(backups) > keep_count:
            logger.info(
                f"Kept {keep_count} most recent backups, removed {len(backups) - keep_count}"
            )
...
Examples:
  ...
  USE_SHARED_ENGINES=true python scripts/backup_tm.py --output backups/tm_20250101.tar.gz
    ...
    parser.add_argument(
        "--config", type=str, default="./config", help="Config directory path. Default: ./config"
    )

    parser.add_argument(
        "--rotate", type=int, help="Keep only last N backups (removes older backups)"
    )
