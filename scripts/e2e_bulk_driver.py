...
def run_verification(source_path: Path, target_path: Path, lang: str) -> Tuple[bool, Dict]:
    ...
    try:
        ...
        if json_output.exists():
            ...
            result_data = {
                'passed': False,
                'errors': ['Verification did not produce JSON output']
            }
...
def main():
    ...
    parser.add_argument('--filelist', default=str(REPORTS_DIR / "E2E_BULK_filelist.txt"),
                        help='File list to process')
    parser.add_argument('--langs', nargs='+', default=['fr', 'de'],
    ...
    with open(CONSOLE_LOG, 'w', encoding='utf-8') as f:
        f.write(f"E2E BULK TRANSLATION CONSOLE LOG\n")
    ...
    for file_idx, source_path in enumerate(files_to_process, 1):
        ...
        for lang in args.langs:
            ...
            # Run translation
            translate_success, translate_output = run_translation(
                source_path, lang, site_profile
            )
            ...
            # Run verification
            verify_success, verify_result = run_verification(
                source_path, target_path, lang
            )
