...
def check_opus_model_exists(
    src_lang: str, tgt_lang: str, timeout: int = 10
) -> tuple[bool, str | None]:
    """
...
def generate_opus_model_entry(
    src_lang: str, tgt_lang: str, check_online: bool = False
) -> dict | None:
    """
    ...
    if check_online:
        ...
        if not exists:
            ...
            logger.warning(f"Skipping {model_id}: {error or 'not available'}")
...
def main() -> int:
    ...
    for tgt_lang in target_langs:
        entry = generate_opus_model_entry(
            src_lang=args.source_lang, tgt_lang=tgt_lang, check_online=args.check_online
        )
