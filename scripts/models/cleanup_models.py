...
def main():
    ...
    if args.model:
        ...
        models_to_delete = manager.find_cleanup_candidates(
            min_size_mb=args.min_size, days_since_access=args.days_unused
        )
    ...
    if args.dry_run:
        if args.json:
            ...
            for model in models_to_delete:
                ...
                accessed = (
                    f"(last accessed: {model.last_accessed[:10]})"
                    if model.last_accessed
                    else "(never accessed)"
                )
                print(
                    f"  - {model.model_id:<30} {model.size_mb:>8.1f} MB {backend_info} {accessed}"
                )
            print()
