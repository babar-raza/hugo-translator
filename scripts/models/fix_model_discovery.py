...
def create_updated_registry(scan_results: dict, old_registry_path: Path, output_path: Path) -> None:
    """
    ...
    for model in models:
        ...
        if new_path:
            ...
            logger.warning("  ⚠ No path found and no HF ID!")
...
def cleanup_incomplete_models(scan_results: dict, models_dir: Path, dry_run: bool = True) -> None:
    ...
    for info in incomplete:
        size_mb = sum(f.stat().st_size for f in info["path"].rglob("*") if f.is_file()) / (
            1024 * 1024
        )
        total_size += size_mb
...
def generate_report(scan_results: dict, output_file: Path) -> None:
    ...
    for info in sorted(complete, key=lambda x: x["rel_path"]):
        report.append(
            f"| {info['rel_path']} | {info['type']} | {info['file_count']} | {info['size_mb']} |\n"
        )
    ...
    for info in sorted(incomplete, key=lambda x: x["rel_path"]):
        report.append(
            f"| {info['rel_path']} | {info['file_count']} | Missing config.json or weights |\n"
        )
...
def main():
    ...
    logger.info("3. If satisfied, run with --cleanup to remove incomplete models")
    logger.info("4. Replace config/model_registry.yaml with the fixed version")
    logger.info("5. Run E2E tests to verify")
