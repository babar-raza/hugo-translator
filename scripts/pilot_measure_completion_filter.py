...
for cr_str in raw.get('content_roots', []):
    ...
    print(f"  AFTER  (with completion filter):   {incomplete} include, {complete} skip ({pct}% skipped)")
    print(f"  Net savings per daemon run: {complete} file-translate calls avoided")
    if complete > 0:
        print(f"  Alphabetical trap status: ", end="")
        ...
        for f in first20:
            ...
            if all_c:
                first20_complete += 1
        print(f"{first20_complete}/20 of alphabetically-first files were complete (previously consumed max_files slots every run)")
