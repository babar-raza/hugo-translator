...
from src.translation_engine.parser.ast_nodes import NodeType, ASTNode
...
def check_duplicate_translations(headings: List[str], threshold: int = 3) -> Tuple[bool, Dict[str, int]]:
    """
    ...
    LEGITIMATE_REPEATED = {
        # English
        'parameters', 'returns', 'examples', 'remarks', 'see also',
        'constructors', 'properties', 'methods', 'events', 'fields',
        'property value', 'return value', 'exceptions', 'type parameters',
        # French
        'paramètres', 'retour', 'exemples', 'remarques', 'voir aussi',
        'constructeurs', 'propriétés', 'méthodes', 'événements', 'champs',
        'valeur propriété', 'valeur de retour', 'exceptions',
        # Russian
        'параметры', 'возвращение', 'примеры', 'замечания', 'смотрите также',
        'конструкторы', 'свойства', 'методы', 'события', 'поля',
        'стоимость недвижимости', 'возвращаемое значение', 'исключения',
        # German
        'parameter', 'rückgabe', 'beispiele', 'hinweise',
        # Spanish
        'parámetros', 'retorno', 'ejemplos', 'observaciones',
    }
...
def analyze_structure(source_file: Path, target_file: Path, target_lang: str) -> Dict[str, Any]:
    ...
    try:
        ...
        important_types = ['HEADING', 'PARAGRAPH', 'LIST', 'LIST_ITEM', 'STRONG',
                          'EMPHASIS', 'CODE_SPAN', 'LINK', 'IMAGE', 'CODE_BLOCK']
        ...
        checks['structure_analysis_error'] = {
            'passed': False,
            'error': str(e),
            'concern': 1
        }
...
def analyze_language(target_file: Path, target_lang: str) -> Dict[str, Any]:
    ...
    try:
        ...
        latin_script_langs = {'fr', 'de', 'es', 'it', 'pt', 'nl', 'pl', 'ro', 'cs', 'sk',
                              'hr', 'sl', 'hu', 'sv', 'da', 'no', 'fi', 'et', 'lt', 'lv',
                              'ca', 'eu', 'gl', 'id', 'ms', 'vi', 'tr'}
        ...
        checks['language_analysis_error'] = {
            'passed': False,
            'error': str(e),
            'concern': 3
        }
...
def generate_markdown_report(results: Dict[str, Any], output_path: Path):
    ...
    lines = []
    lines.append(f"# Translation Analysis Report")
    lines.append(f"")
    ...
    if problem_rate <= 0.02:
        lines.append(f"**VERDICT**: ✅ **PASS** (Problem Rate ≤ 2%)")
    else:
        lines.append(f"**VERDICT**: ❌ **FAIL** (Problem Rate > 2%)")
    lines.append(f"")

    # Concern breakdown
    lines.append(f"## Concerns Breakdown")
    lines.append(f"")
    lines.append(f"| # | Concern | Total | Passed | Failed | Rate |")
    lines.append(f"|---|---------|-------|--------|--------|------|")
    ...
    for file_key, file_data in results['files'].items():
        ...
        for lang, lang_data in file_data.items():
            ...
            if passed_checks:
                lines.append(f"**Passed Checks ({len(passed_checks)}):** {', '.join(passed_checks.keys())}")
                lines.append(f"")

            if not failed_checks and not passed_checks:
                lines.append(f"No checks performed")
                lines.append(f"")
...
def main():
    ...
    parser.add_argument('--target-langs', nargs='+', default=['fr', 'ru'],
                       help='Target languages to analyze')
    parser.add_argument('--run-id', default='analysis',
                       help='Run identifier for reports')
    ...
    print(f"Analyzing translation output...")
    ...
    if results['summary']['problem_rate'] <= 0.02:
        print(f"[PASS] Problem Rate <= 2%")
        return 0
    else:
        print(f"[FAIL] Problem Rate > 2%")
        print(f"")
        print(f"Failed Concerns:")
        for i in range(1, 11):
