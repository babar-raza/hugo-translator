...
"""
    ...
    results = {
        ...
        "total_pipeline": {
            "mean_ms": statistics.mean(parse_times) + statistics.mean(extract_times) + statistics.mean(reconstruct_times),
        }
    }

    print(f"  Parse: {results['parse']['mean_ms']:.3f}ms (p95: {results['parse']['p95_ms']:.3f}ms)")
    print(f"  Extract: {results['extract']['mean_ms']:.3f}ms (p95: {results['extract']['p95_ms']:.3f}ms)")
    print(f"  Reconstruct: {results['reconstruct']['mean_ms']:.3f}ms (p95: {results['reconstruct']['p95_ms']:.3f}ms)")
...
""" + "\n\n".join([f"## Section {i}\n\nThis is paragraph {i} with content." for i in range(50)])
    ...
    for size_name, content in [("small", small_file), ("medium", medium_file), ("large", large_file)]:
        ...
        print(f"  {size_name.capitalize()} ({len(segments)} segments): {results[size_name]['mean_ms']:.2f}ms ({results[size_name]['ms_per_segment']:.2f}ms/segment)")
