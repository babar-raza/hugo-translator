...
def print_report(results: Dict[str, Dict[str, List[bool]]], threshold: float = 0.80) -> None:
    ...
    for lang, lang_data in sorted(results.items()):
        ...
        flag = "[!]" if low else "   "
        print(f"  {flag} {lang:<6}  overall={overall:.1%}  ({len(low)} terms below {threshold:.0%})")
...
def main() -> int:
    ...
    p.add_argument("--langs", nargs="+", default=["fr", "de", "es", "ar", "zh"],
                   help="Target language codes to check")
    p.add_argument("--n-sample", type=int, default=20,
                   help="Files to sample per language (default: 20)")
