...
class SpecScanner:
    ...
    def _validate_start_method(self, command: str) -> bool:
        ...
        if command.startswith('python -m '):
            ...
            return bool(re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*(\.[a-zA-Z_][a-zA-Z0-9_]*)*$', module_path))
    ...
    def _verify_feature(self, file_path: Path) -> SpecScanResult:
        ...
        if not re.search(r'^#\s+' + re.escape(feature_id.upper().replace('-', '-')), content, re.MULTILINE | re.IGNORECASE):
    ...
    def _generate_report(self) -> Path:
        ...
        lines.append("")
        pass_pct = (self.summary.passed / self.summary.total_specs * 100) if self.summary.total_specs > 0 else 0
        fail_pct = (self.summary.failed / self.summary.total_specs * 100) if self.summary.total_specs > 0 else 0
        warn_pct = (self.summary.warnings / self.summary.total_specs * 100) if self.summary.total_specs > 0 else 0

        lines.append(f"- ✅ **Passed:** {self.summary.passed}/{self.summary.total_specs} ({pass_pct:.1f}%)")
        lines.append(f"- ❌ **Failed:** {self.summary.failed}/{self.summary.total_specs} ({fail_pct:.1f}%)")
        lines.append(f"- ⚠️  **Warnings:** {self.summary.warnings}/{self.summary.total_specs} ({warn_pct:.1f}%)")
        lines.append("")
        ...
        for spec_type, results in sorted(results_by_type.items()):
            ...
            lines.append(f"**Total:** {len(results)} | **Passed:** {passed} | **Failed:** {failed} | **Warnings:** {warnings}")
            lines.append("")
                ...
                if result.status == "FAIL":
                    for msg in result.messages:
                        if "Evidence ID not found" in msg:
                            ...
                            recommendations.append(f"Add missing field '{field}' in {result.spec_id}")
