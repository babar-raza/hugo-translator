...
class QualityBenchmark:
    ...
    def load_reference_corpus(self, corpus_path: Path) -> list[dict]:
        ...
        logger.info(
            f"Loaded {data['sample_count']} samples for {data['language_pair']}"
        )
        return data["samples"]
    ...
    def benchmark_model(
        ...
        logger.info(
            f"Benchmarking {model_id} on {len(corpus_samples)} samples..."
        )
        ...
        for i, sample in enumerate(corpus_samples):
            ...
            try:
                ...
                if hasattr(backend, 'translate'):
                    ...
                    translation = translation_result[0] if isinstance(translation_result, list) else translation_result
                else:
                    ...
                    translation = backend.tokenizer.decode(
        ...
        run = BenchmarkRun(
            ...
            metadata={
                ...
                "avg_comet": sum(r.comet_score for r in results if r.comet_score is not None) / len([r for r in results if r.comet_score is not None]) if any(r.comet_score for r in results) else None,
            },
