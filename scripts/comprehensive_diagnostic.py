    ...
    try:
        ...
        for lang_code, lang_name in test_langs.items():
            ...
            for i, text in enumerate(test_texts, 1):
                ...
                status = "✓ CLEAN" if not contaminants else f"✗ CONTAMINATED ({', '.join(contaminants)})"
                try:
        ...
        for lang_code in test_langs.keys():
            ...
            for trans in individual_results.get(lang_code, []):
                if any(marker in trans for marker in ['Zkouška', 'Übersetzer', 'ΠΡΟΣΟΧΗ', 'Arxius', 'تخزين']):
        ...
        print(f"Batch mode contaminated: {list(batch_contaminated) if batch_contaminated else 'None'}")
