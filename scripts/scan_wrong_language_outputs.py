...
def scan_site_lang(
    ...
    logger.info(f"[{site_id}/{target_lang}] Checked {checked} files, flagged {len(flagged)} wrong-language")
    return flagged
...
def print_results_table(flagged: list[dict]) -> None:
    ...
    for site_id, items in sorted(by_site.items()):
        ...
        for item in items[:3]:
            print(f"  [{item['expected_lang']}→detected:{item['detected_lang']} conf:{item['confidence']:.2f}] {item['output_path']}")
        if len(items) > 3:
...
def main() -> None:
    ...
    if not profiles:
        print(f"No profiles found" + (f" for '{args.site}'" if args.site else ""), file=sys.stderr)
        sys.exit(1)
