...
def compute_expected_translation_path(source_file: Path, source_lang: str, target_lang: str, content_root: str) -> Path:
    """
...
def discover_site(site_profile_path: str, run_dir: Path) -> Dict:
    ...
    print(f"Target languages: {len(target_langs)} ({', '.join(target_langs[:5])}{'...' if len(target_langs) > 5 else ''})")
    ...
    for source_file, content_root in all_source_files:
        for target_lang in target_langs:
            target_path = compute_expected_translation_path(source_file, source_lang, target_lang, content_root)
    ...
    with open(manifest_csv, 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=['site_id', 'source_file', 'target_lang', 'target_path', 'status', 'content_root'])
        writer.writeheader()
