...
def print_status(status_list: list[dict]) -> None:
    ...
    print("-" * 70)
    print(f"Total: {len(status_list)} sites, {enabled_count} enabled, {len(status_list) - enabled_count} disabled\n")
...
def main():
    ...
    parser.add_argument(
        "--site",
        type=str,
        help="Site ID to modify (e.g., products.aspose.net)"
    )

    parser.add_argument(
        "--all-sites",
        action="store_true",
        help="Apply to all site profiles"
    )

    parser.add_argument(
        "--enable",
        action="store_true",
        help="Enable AST translation"
    )

    parser.add_argument(
        "--disable",
        action="store_true",
        help="Disable AST translation"
    )

    parser.add_argument(
        "--status",
        action="store_true",
        help="Show status of all sites (no modifications)"
    )
    ...
    for profile in target_profiles:
        ...
        if changed:
            ...
            print(f"{status} {site_id:<40} {'WOULD BE ' if args.dry_run else ''}{'ENABLED' if enable else 'DISABLED'}")
            modified_count += 1
        else:
