...
def _gather_sample_files(content_root: Path, n: int = FILES_PER_SITE) -> list[Path]:
    ...
    candidates = [
        ...
        if not any(f.stem.endswith(f".{lc}") for lc in SAMPLE_LOCALES) and f.suffix == ".md"
    ]
    return candidates[:n]
...
def _verify_idempotent(all_results: list[dict]) -> dict:
    ...
    select_count = sum(
        1 for r in all_results if r.get("aggregate") in ("SELECT_MISSING", "SELECT_STALE")
    )
    return {
        "description": (
...
def main() -> int:
    ...
    for site_id in target_sites:
        ...
        file_results = [classify_file(src, SAMPLE_LOCALES, site_id, cr) for src in sample_files]
    ...
    report["scenarios"] = {
        ...
        "D_all_sites_covered": {
            ...
            "sites_sampled": [s for s in target_sites if "error" not in report["sites"].get(s, {})],
            "passed": all("error" not in report["sites"].get(s, {}) for s in target_sites),
        },
