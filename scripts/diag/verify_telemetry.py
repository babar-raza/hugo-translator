...
TELEMETRY_SRC_PATH = Path(
    os.getenv("TELEMETRY_SRC_PATH", r"C:\Users\prora\OneDrive\Documents\GitHub\local-telemetry\src")
)
...
def verify_latest_record():
    ...
    check("host", record.get("host"), record.get("host") is not None, "Should be hostname")

    # Standard fields
    print("\n--- Standard Fields ---")
    check("run_id", record.get("run_id"), record.get("run_id") is not None, "Required")
    check("agent_name", record.get("agent_name"), record.get("agent_name") is not None, "Required")
    check("job_type", record.get("job_type"), record.get("job_type") is not None, "Required")
    check(
        "trigger_type",
    ...
    check("start_time", record.get("start_time"), record.get("start_time") is not None, "Required")
    check(
        "status",
        record.get("status"),
...
def main():
    ...
    engine = TranslationEngine(
        config_service=config_service, tm=tm, model_loader=model_loader, enable_telemetry=True
    )

    # Translate one file to one language
    test_file = Path(
        r"D:\onedrive\Documents\GitHub\aspose.net\content\products.aspose.net\slides\en\_index.md"
    )
