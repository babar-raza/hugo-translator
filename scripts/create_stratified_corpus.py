...
ASPOSE_FAMILIES = [
    'slides', 'words', 'cells', 'pdf', 'email', 'tasks', 'barcode', 'imaging',
    'diagram', 'note', 'html', '3d', 'cad', 'gis', 'zip', 'tex', 'psd', 'omr',
    'svg', 'finance', 'drawing', 'font', 'page', 'ocr', 'total', 'annotation'
]
...
def extract_family_from_path(path: Path) -> Optional[str]:
    """
...
def extract_subdomain_from_path(path: Path) -> Optional[str]:
    """
...
def collect_markdown_files(
    ...
    include_subdomains: Optional[List[str]] = None,
) -> List[Path]:
    """
...
def create_stratified_sample(
    ...
    logger.info(f"Stratification: {len(ASPOSE_SUBDOMAINS)} subdomains × {len(ASPOSE_FAMILIES)} families")
