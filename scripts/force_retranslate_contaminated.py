...
def _already_queued_paths() -> set[str]:
    ...
    try:
        ...
        logger.warning(f"Could not load existing retranslate queue: {e}. Skipping idempotency check.")
        return set()
...
def main():
    ...
    parser.add_argument(
        '--site', default=None,
        help='Filter to one site_id (e.g. --site blog.aspose.net)'
    )
    parser.add_argument(
        '--lang', default=None,
        help='Filter to one target language (e.g. --lang de)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--delete-outputs', action='store_true',
    ...
    for record in candidates:
        ...
        if args.dry_run:
            logger.info(f"[DRY-RUN] Would queue: {output_path.name} ({tgt_lang}, purity={purity:.1f}%)")
        else:
    ...
    # Summary
    print(f"\n{'='*60}")
    print(f"Force Retranslate — Summary")
    print(f"{'='*60}")
    ...
    if args.dry_run:
        ...
        print(f"Deleted:            {n_deleted}" + (" (--delete-outputs)" if args.delete_outputs else ""))
        print(f"Errors:             {n_failed}")

    if not args.dry_run and n_queued > 0:
        print(f"\nNext step: Run the content worker to retranslate queued files.")
        print(f"  python -m src.workers.autonomous_content_translation_worker --mode oneshot --site <site>")
        print(f"\nRecommendation: Run L2 cache audit first if not already done:")
        print(f"  python scripts/audit_l2_cache_contamination.py --repair")
